...
> **Kubernetes 系列 · 第 30/30 篇（完结）**  
> 上一篇：[《项目上云实战——Java/Python/Golang 与中间件部署》](/云原生/k8s/k8s-29-app-onboarding)

---

## 开头：HPA 只看 CPU/QPS，队列积压了怎么办

[第 08 篇](/云原生/k8s/k8s-08-hpa-cri-crd)我们用 HPA 按 CPU/内存扩缩容，[第 18 篇](/云原生/k8s/k8s-18-prometheus-hpa)接上 Prometheus 后按 QPS 扩，[第 19 篇](/云原生/k8s/k8s-19-custom-metrics)又打通了自定义指标。但生产里还有一类场景它们都不擅长：
...
| Prometheus + HPA | [18 篇](/云原生/k8s/k8s-18-prometheus-hpa) | Prometheus 采集的 QPS 等标准指标 | 指标仍以「应用自身」为中心 |
| 自定义指标 HPA | [19 篇](/云原生/k8s/k8s-19-custom-metrics) | custom-metrics API（任意 PromQL） | 配置链路长；仍难直接表达「队列长度→副本数」 |
...
> 💡 **选型建议**：HTTP add-on 适合无现成指标可用的通用 Web 流量；如果集群已有 Prometheus（[18 篇](/云原生/k8s/k8s-18-prometheus-hpa)的栈），用 `prometheus` scaler 直接写 PromQL 更轻，无需额外组件；消息消费类 workload 则首选 `kafka`/`rabbitmq` scaler。
...
metrics-server 只保留**近实时瞬时值**（默认 15s 采样、内存中短暂留存），没有历史——回答不了「这台节点上周 CPU 是多少」。生产级主机监控是本系列 [18 篇](/云原生/k8s/k8s-18-prometheus-hpa)已部署的 **node-exporter + Prometheus + Grafana** 组合：
...
在 18 篇环境里，node-exporter 已随 kube-prometheus-stack 以 DaemonSet 形式跑在每个节点，Grafana 导入社区 `Node Exporter Full` 大盘即可看到节点级历史曲线——这正是本篇 KEDA 压测时观察「扩容是否把节点打满」的最佳工具。指标链路的更多玩法（ServiceMonitor、自定义指标）见 [18 篇](/云原生/k8s/k8s-18-prometheus-hpa)与 [19 篇](/云原生/k8s/k8s-19-custom-metrics)。

> 💡 三层监控各司其职：**metrics-server** 喂 HPA 和 `kubectl top`（瞬时）、**node-exporter + Prometheus** 记历史（趋势）、**日志（ELK/EFK，见[24 篇](/云原生/k8s/k8s-24-logging-elk-efk)）** 查根因。别指望一个工具包打天下。

---

## 四、系列收官总结

30 篇到这里全部完成。呼应 [20 篇](/云原生/k8s/k8s-20-jvm-in-container)末尾的学习路径，把整个系列串成一张总表——**01–20 是概念主线（学「是什么」），21–30 是实践篇（学「怎么落地」）**：
...
| 网络与存储 | [09 Service](/云原生/k8s/k8s-09-service-l4/) / [10 网络与 DNS](/云原生/k8s/k8s-10-network-dns/) / [11 PV/PVC](/云原生/k8s/k8s-11-pv-pvc/) / [12 Ingress](/云原生/k8s/k8s-12-ingress-l7/) | 四层/七层、CNI、存储卷 | 流量入口与持久化 |
| 发布与网格 | [13 发布策略](/云原生/k8s/k8s-13-release-strategies/) / [14 Istio](/云原生/k8s/k8s-14-service-mesh-istio/) | 滚动/金丝雀/蓝绿、灰度治理 | 应用安全上线 |
| 镜像与配置 | [15 Harbor 与 SpringCloud](/云原生/k8s/k8s-15-harbor-springcloud/) / [16 Secret/ConfigMap](/云原生/k8s/k8s-16-secret-configmap/) | 私有仓库、配置注入 | 交付物与配置管理 |
| 自动化与可观测 | [17 Jenkins 金丝雀](/云原生/k8s/k8s-17-jenkins-canary/) / [18 Prometheus+HPA](/云原生/k8s/k8s-18-prometheus-hpa/) / [19 自定义指标](/云原生/k8s/k8s-19-custom-metrics/) / [20 JVM 容器化](/云原生/k8s/k8s-20-jvm-in-container/) | CI/CD、监控、QPS 弹性、运行时调优 | 从部署到观测闭环 |
| 实践篇 21–30 | 21–23 实操进阶 · [24 日志 ELK/EFK](/云原生/k8s/k8s-24-logging-elk-efk) · 25–28 生产专题 · [29 项目上云](/云原生/k8s/k8s-29-app-onboarding) · **30 本篇** | 把主线概念逐一落到生产 | 真正的落地能力 |
