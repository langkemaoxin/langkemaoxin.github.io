...
> **UAC（用户账户控制）**是本机上的一套约定：管理员登录后，**默认先当普通人干活**；只有你明确同意（或主动「以管理员身份运行」），**新进程**才换管理员票去干重活。  
...
| UAC 在做的事 | 你平时怎么感觉到 |
|--------------|------------------|
| 给管理员准备**两张票**，桌面默认用日常票 | 登录后浏览器、Office、普通 CMD 并不自动「通杀」系统目录 |
| 要抬权时**征求同意** | 弹出「是否允许此应用对设备进行更改？」；或右键「以管理员身份运行」 |
| **按进程**发管理员票 | 点「是」只抬**这一次新开的程序**；旁边未提升的窗口不会一起变神 |
| 和门上的 ACE **一起**生效 | 即便已提升，若某文件夹对你 Deny，普通打开路径仍可能进不去（故事一另论备份权利） |
...
| 它不负责 | 那是谁的事 |
|----------|------------|
| 决定「这个文件夹谁能读」 | 对象权限 / ACE（[第 9～10 站](../vol1-invent/10-ace-dacl.md)） |
| 决定「准不准备份整盘、当服务登录」 | 用户权利 / privileges（本站故事一） |
| 网上证明「我是域里的谁」 | 登录 + Kerberos 等（[第 4](../vol1-invent/05-logon-lsa.md)、[16 站](../vol2-identity/02-kerberos.md)） |
| 当你的「权限总开关面板」 | 关 UAC 只会让双票对照变淡，**学不会** ACL，也不等于权限模型更简单 |

小王记一句就够：

> UAC **能**做的是：默认降权 + 要干重活再问你一声 + 只给同意过的新进程管理员票。  
> UAC **不能**单独保证「文件一定安全」——门上的字、共享两道门、用户权利，还在各自的轨道上。
...
第 [5 站](../vol1-invent/06-access-token.md) 说过：令牌挂在**进程**上，里面至少有「你是谁、你带哪些组、你有哪些特权」。  
...
小王能保存，只说明：**这一次写入，通过了「当前进程令牌 vs 该文件 ACL」的检查**（以及若走共享还会有两道门，见 [第 10 站](../vol1-invent/11-access-check.md)）。常见三种故事：
...
- [第 5 站](../vol1-invent/06-access-token.md)：权限检查看的是**进程令牌**，不是「账户名片上写没写管理员」。  
- [第 16 站](../vol2-identity/02-kerberos.md)：票据解决**网上如何证明你是谁**；UAC 解决**本机进程揣哪张通行证**。别混成一个词。
...
| 和前几站 | ACE 仍重要，但不唯一 | [第 5 站](../vol1-invent/06-access-token.md)「令牌挂在进程上」在这里落地 |
...
系列正文到此。请到 [总图](../appendix/01-map.md) 串线复习，或回 [索引](../00-index.md) 跳站；更多出处见 [参考](../appendix/05-references.md)。
