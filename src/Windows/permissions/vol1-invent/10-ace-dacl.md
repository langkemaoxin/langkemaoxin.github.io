...
本讲重点是「谁能碰这个对象」那张表，也就是安全描述符里的 **DACL（Discretionary ACL）**。  
（审计用的 SACL 以后再讲；完整「安全描述符拼图」在第 11 讲。）
...
> 人带着令牌去敲门；门上贴的是 DACL。  
> 本讲先把「门上的字」写清楚；下一讲再讲系统怎么对照令牌读这些字。
...
（本讲只需要「NTFS = Windows 默认文件系统，能挂安全描述符」。加密、集群卷等其它 NTFS 特性这里不展开。）
...
在进入第 10 讲「完整对表」之前，先把 Appendix B 里的判定直觉立住：
...
ACE 常写给组，而不是写给每个人。你令牌里带着组 SID（第 5、8 讲），就会命中「授给该组」的那一行。
...
### 9.6 Permissions ≠ User rights（本讲只点破，不展开）
...
> **DACL 很重要，但不是宇宙尽头。**  
> 「权利压过权限 / 夺所有权」的细节 → **第 20 讲**再讲透。

（Appendix B 后半关于 Enterprise Admins、Domain Admins 等内置高权组的大表，属于域与高权专题，**不在本讲展开**。）
...
```bat
:: 查看文件的权限（真实共享样例；第 10 讲会用这份 DACL 与 whoami /groups 对表）
...
来源：[icacls](https://learn.microsoft.com/en-us/windows-server/administration/windows-commands/icacls)（`/grant`、`/deny`、`/save`、`/restore`；**完整标志与实测见第 12 讲**）
...
（继承相关参数下一讲之后的「继承专章」再加；这里先写「作用于当前对象」的最简 ACE。）
...
**下一讲才需要：** 打开文件时，系统如何拿你令牌里的 SID，去和这张 DACL **逐条对表**（Access Check）。
