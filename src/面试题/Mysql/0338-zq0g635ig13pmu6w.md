---
title: "100W数据去重，该用distinct还是group by，说说理由？"
sidebarGroup: "Mysql"
shortTitle: "100W数据去重，该用distinct还是group by，说说理由？"
order: 338
date: 2026-07-17
category: "面试题"
tag:
  - "面试题"
description: "对于100万级别的数据去重，选择 DISTINCT 还是 GROUP BY，在功能上（仅去重，不进行聚合运算）两者是等效的，大多数现代数据库的查询优化器会将它们生成相似甚至相同的执行计划。然而，从可读性和表达意图的角度来看，DISTINCT"
article: false
---

> 来源：[100W数据去重，该用distinct还是group by，说说理由？](https://www.yuque.com/tulingzhouyu/db22bv/zq0g635ig13pmu6w)
...
- `GROUP BY`: 这个子句的主要目的是**将数据分成多个组，并对每个组进行聚合操作**（如 `COUNT()`, `SUM()`, `AVG()` 等）。虽然在不使用聚合函数时，`GROUP BY` 可以达到去重的效果，但这并**非其主要设计目的**。如果读者看到 `GROUP BY`，通常会期望看到聚合函数。
