...
> **Docker 系列 · 第 23/24 篇**
> 上一篇：[《容器安全——Capabilities 降权、Seccomp 与不该用的 --privileged》](/云原生/docker/docker-22-container-security) · 下一篇：[《Daemon 运维——daemon.json、live-restore 与远程管理》](/云原生/docker/docker-24-daemon-ops)
...
问题出在哪、怎么把镜像压到 **20MB（72 倍瘦身）**、构建缓存怎么配才能「改一行代码秒级重建」——本篇在本机（Docker 29.x，WSL2）从反面教材开始实测。前置知识：[第 9 篇 Dockerfile 基础](/云原生/docker/docker-09-dockerfile/)、[第 17 篇分层与缓存](/云原生/docker/docker-17-unionfs/)。
...
[第 17 篇](/云原生/docker/docker-17-unionfs/)讲过「指令一层、上层变则下层全部重建」。原样重建上面的多阶段 Dockerfile，看真实输出：
