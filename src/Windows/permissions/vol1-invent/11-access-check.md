...
- 无匹配 ACE → 不能访问；Deny 通常压过 Allow（第 9 讲）
...
**网络登录**与**共享权限**紧接着在 10.2 / 10.3 补上——读完本讲才算把 UNC 打开路径讲完整。
...
怎么读这些记号（完整表与实测见**第 12 讲**；本处够用）：

| 记号 | 含义 |
|------|------|
| `(I)` | 继承来的 ACE |
| `(OI)(CI)` | 可继续向子文件/子文件夹继承（第 12 讲细讲） |
| `(F)` | 完全控制 |
...
若换一个**令牌里既不在项目组、也不在只读组、也不在设总**的域账户去开同一路径：DACL 上没有匹配 ACE → 按第 9 讲直觉 → **不能访问**（表现为拒绝访问），除非共享/NTFS 上另有其它命中规则。
...
域账户与「票据怎么换」放到第 15、16 讲；此处先接受：**UNC 打开前，服务器必须先认清你是谁。**
...
**下一讲才需要：** Owner 与 DACL 在对象上如何放进同一份安全描述符结构里。
