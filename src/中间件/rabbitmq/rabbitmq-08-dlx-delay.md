...
**没有定时器盯着每条消息倒计时。** 百万消息就要百万个定时器，Broker 早就被拖垮了。实际的机制是**打戳 + 懒检查**：

**第一步——打戳（入队时）**：消息进入有 TTL 的队列时，Broker 给它算一个**过期时刻** = `入队时刻 + TTL`，记在消息元数据上。到这一步就结束了，没有任何线程在"等"。

**第二步——懒检查（只查队头）**：Broker **只在消息排到队头**（队列最老的位置）时，才查它有没有过期。查的过程：

| 时刻 | 队头消息 | 过期了吗 | Broker 做什么 |
|------|----------|----------|---------------|
| 到期时刻刚过 | 消息 A（已过期） | ✅ 过期 | 死信掉，下一条冒上来当新队头，**立刻再查** |
| 紧接着 | 消息 B（还没过期） | ❌ 没过期 | 不动，等下一轮周期再查 |

> 关键：**前一条过期、被死信后，下一条立刻被查**——所以如果连续几条同时过期，它们会一连串被死信。但如果队头没过期，后面的消息即使已过期了也**不会被查到**（被没过期的队头挡着）。

**第三步——周期性收割**：即使延迟队列没有消费者、队头消息迟迟不"被触发"，Broker 也会**周期性地**主动扫一次队头，把过期的收割掉。所以延迟队列**不挂消费者也能正常到期**。

**后果——延迟不精确**：消息可能在**理论到期时刻之后一小会儿**才真正被死信（它得等前面的消息先被查完、自己冒泡到队头）。所以 TTL + DLX 的延迟是**秒级精度**，不是毫秒级。
