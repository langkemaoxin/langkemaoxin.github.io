...
> **Docker 系列 · 第 17/24 篇**
> 上一篇：[《Docker 技术底座》](/云原生/docker/docker-16-tech-foundation) · 下一篇：[《构建进阶——同一个镜像从 1.44GB 瘦到 20MB》](/云原生/docker/docker-18-build-advanced)
>
> 原理篇第 2 篇：上一篇认了三根支柱的脸，这一篇拆第一根。

---

## 写在前面

写 Docker 系列写到原理篇，[第 16 篇](/云原生/docker/docker-16-tech-foundation)认了 Namespace / Cgroups / UnionFS 三根支柱。前两根都好懂，唯独 UnionFS 让我发怵——「联合文件系统」几个字拆开都认识，合起来不知道在说什么。而镜像的种种好处全挂在它身上：400 MB 的镜像 10 台节点共用、改一行代码重建只要几秒，凭什么？
...
（对照：[第 12 篇](/云原生/docker/docker-12-data-persistence)的 bind mount 是把**一个**目录接到一个路径上；这里是把**多个**目录叠成一个视图，是它的加强版。）
...
**🧑‍🏫 老师：** 省不下来——挡板只遮视图，原件还躺在楼下占着地方。同理，`rm` 之后 `docker commit` 成新镜像，文件在新镜像里「不在」了，空间仍在底下的层里。真要瘦身，得从 Dockerfile 里去掉这个文件重新 build——多阶段构建就是干这个的，先埋个伏笔，[第 18 篇](/云原生/docker/docker-18-build-advanced)展开。
...
多阶段构建、BuildKit 缓存挂载这些深水区，[第 18 篇构建进阶](/云原生/docker/docker-18-build-advanced)展开。
...
传统 Docker Engine 直装 Linux 时这里多显示 `overlay2`；Engine 29 起新装默认启用 containerd 镜像存储（containerd 是 Docker 内部管镜像和容器的组件，[第 21 篇](/云原生/docker/docker-21-daemon-runtime)展开），同一个 OverlayFS 机制的显示名变成 `overlayfs`。老资料里的 `devicemapper` 同样早已不推荐。**实现换了几茬，「只读层垫底 + 可写层在顶 + 写时复制 + whiteout」的语义一步没变**——这也是本篇敢拿 OverlayFS 手工实验代替老教程 AUFS 实验的原因。
