...
第 4、5 讲：坐在电脑前登录 → 本机得到 **Access Token**。  
第 10 讲：打开 `\\jzfz18\协同设计平台-18\...` 时，还要做**网络登录**——服务器也得认清你。
...
> 本讲严格西蒙节奏：先发明「盖章处 / 总通行证 / 专用票」，再贴 Kerberos 官方名。  
> **不展开**黄金票据、委派、加密套件考试表。
...
域里已经有第 15 讲的**答账服务器（域控）**。再给它一个额外职责：当**可信的第三方盖章处**。
...
第 3 讲 LSA 图里的 **KDC** 箭头，指的就是这条路上的盖章处。
...
【坐在电脑前登录域账户】
  输入域名\账户 + 密码
    → LSA 联系域（第 4 讲）
    → KDC 认可后：本机得到 Access Token（第 5 讲）
                   并且常缓存一张 TGT（总通行证）

【稍后打开 \\fileserver\share】
  本机用 TGT 向 KDC 申请「访问 fileserver 上 SMB」的专用票
    → 带着专用票去做网络登录（第 10.2 讲）
    → 服务器认可你是谁之后
    → 再走共享门 ∩ NTFS 门（令牌 SID 对 ACE，第 10 讲）
```
...
运维里还会听到 KRBTGT 账户等——那是 KDC 签总票时用的特殊账户，本讲只需知道：**票的信用绑在域的盖章体系上，不绑在某一台文件服务器的良心上。**  
...
**下一讲才需要：** 「能备份整盘」这类能力，和「某个文件 ACE」不是同一类设置；以及管理员为何还弹 UAC。
