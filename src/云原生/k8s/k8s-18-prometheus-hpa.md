...
默认 Prometheus 使用 `emptyDir`，Pod 重建丢数据。生产应在 `Prometheus` CR 中配置 `storage.volumeClaimTemplate` + StorageClass（下一篇运维向内容会展开）。
...
下一篇：**ServiceMonitor 让 Prometheus 发现 Spring Boot Pod**，配置 `http_server_requests_per_second` 规则，完成 HPA 与 Grafana QPS 大盘。
