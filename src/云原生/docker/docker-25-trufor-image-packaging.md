...
`build:` 和 `image:` 写在一起，构建产物直接 tag 成 `trufor-api:cvpr2023`——[第 13 篇](/云原生/docker/docker-13-compose)讲过，不写 `image` 时默认是「项目名-服务名」。
...
2. 在 `/etc/docker/daemon.json` 里补 `"dns"`，重启 Docker（怎么改 daemon.json 见[第 24 篇](/云原生/docker/docker-24-daemon-ops)）。

注意 DaoCloud 加速器**仍然有用**——但加速器自己也是个域名，也得先能被解析。这一课对应主线：[第 4 篇 registry-mirrors](/云原生/docker/docker-04-install)、[第 24 篇 daemon.json](/云原生/docker/docker-24-daemon-ops)。**镜像源配了 ≠ 构建网络通。**
...
处理简单粗暴：**再跑一次 `docker compose build`**。已经落盘的层走缓存，不必从头来——[第 23 篇](/云原生/docker/docker-23-build-advanced)讲过的构建缓存，这里成了救命稻草。这是**构建网络问题，不是配方写错**，改 Dockerfile 没用。
...
- **`ports`**：`"${TRUFOR_PORT:-8088}:8088"`，[第 11 篇](/云原生/docker/docker-11-network)的 `-p` 写进 YAML；`:-8088` 这种正数默认值就没那么娇气。
- **`healthcheck`**：容器内自己 `urlopen` 打 `/health`（第 13 篇讲过探针怎么读：`interval` 多久探一次、`retries` 连挂几次算不健康、`start_period` 启动宽限期——模型加载慢，给了 60 秒）。
...
当场效果：一份能 `up` 的完整主文件。改完变量想先核对渲染结果，用[第 13 篇](/云原生/docker/docker-13-compose)的招 `docker compose config` 先看再起。
...
- **`docker-compose.yml` 这个文件名**：V2 的首选名是 `compose.yaml`，老名字 `docker-compose.yml` 也能认（[第 13 篇](/云原生/docker/docker-13-compose)讲过）。交付仓库沿用的是老名字；新项目建议直接用新名字。
...
| [第 24 篇](/云原生/docker/docker-24-daemon-ops) daemon.json | 雪球 3：`"dns"` 补在 daemon.json |
| [第 9 篇](/云原生/docker/docker-09-dockerfile) Dockerfile | 雪球 2/5/6：构建上下文、COPY、保险丝 |
| [第 23 篇](/云原生/docker/docker-23-build-advanced) 构建进阶 | 雪球 4：层缓存救了第二次 build |
| [第 11 篇](/云原生/docker/docker-11-network) 网络 | 雪球 11：`-p` 与 EXPOSE 的区别 |
| [第 13 篇](/云原生/docker/docker-13-compose) Compose | 雪球 9/10：变量、healthcheck、profiles、多文件合并 |
...
- [第 13 篇 Compose](/云原生/docker/docker-13-compose)（变量插值、healthcheck、profiles 的主干课）
