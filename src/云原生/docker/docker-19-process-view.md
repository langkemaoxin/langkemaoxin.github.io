...
> **Docker 系列 · 第 19/24 篇**
> 上一篇：[《Namespace 隔离——从一个容器里的怪现象滚穿六大命名空间》](/云原生/docker/docker-18-namespace) · 下一篇：[《CGroups 限资源——给同一个容器逐项上枷锁》](/云原生/docker/docker-20-cgroups)
...
两棵树形状一模一样。还有个不对称值得记：容器里的 `ps` 只看得到自己 namespace 里的进程，引擎那边却看得到所有容器——`docker top` 看得见你，你看不见别人，隔离是单向的（原理见[第 18 篇](/云原生/docker/docker-18-namespace)）。
...
shim 为什么存在、和 dockerd / containerd / runc 什么关系 → [第 21 篇](/云原生/docker/docker-21-daemon-runtime)（思考题也埋在这）。
...
namespace 管「看得见谁」，cgroup 管「用得了多少」（这套分工[第 16 篇](/云原生/docker/docker-16-tech-foundation) 讲过）。看看这个 nginx 的资源账单挂在哪。本机引擎是 **cgroup v2**，还是借 `--pid=host` 的视角，对 `lab-proc` 的 master：
...
开头 `0::` 是 cgroup v2 统一层级的写法；路径里那串长十六进制就是**完整容器 ID**——和雪球 1、2 里的 `b2e73a660658` 对上了。资源控制同样挂在「这个容器」名下。CPU/内存限额怎么配 → [第 20 篇](/云原生/docker/docker-20-cgroups)（下一篇）。
...
| dockerd → containerd → shim → runc | [第 21 篇](/云原生/docker/docker-21-daemon-runtime) | 雪球 6 的 10395、思考题 |
| Namespace 隔离原理 | [第 18 篇](/云原生/docker/docker-18-namespace)（上一篇） | 雪球 3、5 |
| Cgroups 限资源 | [第 20 篇](/云原生/docker/docker-20-cgroups)（下一篇） | 雪球 8 |
| 技术底座总览 | [第 16 篇](/云原生/docker/docker-16-tech-foundation) | 开头前置 |
...
1. 为什么 `containerd-shim` 在 runc 把容器拉起来之后可以退出「创建动作」，却仍留下一个 shim 进程陪着容器？若没有 shim，dockerd 升级时已运行的容器会怎样？（提示：垫片负责 IO/状态与生命周期解耦——展开见[第 21 篇](/云原生/docker/docker-21-daemon-runtime)。）
2. 两个容器里各有一个 PID 1，宿主机上会撞号吗？用雪球 3 的模型和雪球 5 的两棵树推一推。

下一篇：[《CGroups 限资源——给同一个容器逐项上枷锁》](/云原生/docker/docker-20-cgroups)。
