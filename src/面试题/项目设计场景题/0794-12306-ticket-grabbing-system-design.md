...
shortTitle: "12306抢票系统设计"
order: 794
date: 2026-03-16
category: "面试题"
tag:
  - "面试题"
description: "12306 系统挑战创下全球最大实时票务交易系统世界记录，春运一个月抵欧洲一年。最高可达百万并发，承受了这个世界最高 QPS。网站浏览量一天最高超 1500 亿次，峰值是双 11 的三倍。新闻网链接：平均每小时卖出超58万张春运火车票，12"
article: false
---

> 来源：[12306抢票系统设计](https://www.yuque.com/tulingzhouyu/db22bv/dcufnmtc6ut2f4gt)

# 12306 系统挑战

- 创下全球最大实时票务交易系统世界记录，春运一个月抵欧洲一年。
- 最高可达百万并发，承受了这个世界最高 QPS。
...
![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-3fb404e5397f.png)

# 12306 系统难点

### 海量请求集中爆发

平时QPS较低，基本都集中在临近春节这段时间集中爆发。

（自动扩容不行）

针对这种类似秒杀的抢购场景之前场景已分享，详情课参面试突击宝典：[如何设计一个秒杀系统](https://www.yuque.com/tulingzhouyu/db22bv/zmh8bxm30a2a81p2#CiJNP)

**总的思路**可以从 **拦**截（80-90%） 和 分**摊**来考虑。

**APP端**：开始将请求拦在系统上游 ，可通过JS限时提交次数(误杀) CDN动静分离等

**站点**：用户(uid)标识 Token校验 鉴权防刷 拦截恶意发包. 限流降级，监控预警

**服务**：先到多级缓存(CDN，Nginx，Ehcache，Redis，BufferPool)读写分离，再根据数据库 进行异步分批，削峰填谷( 请求队列批次) 负载均衡等

**数据**：数据库缓存，读写分离，集群，SQL优化，分析型数据库，数据湖

每层都可以考虑分摊，加入负载均衡多网关多中心多数据库实例分流处理

纵向多级缓存策略 层层过滤分摊压力

更多拦截可参看面试突击宝典：[接口被刷百万QPS，怎么防？](https://www.yuque.com/tulingzhouyu/db22bv/sabap6mrnfbv92gf)

![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-3961a793cf0a.png)

### 库存扣减

它跟传统的电商网站，可能最大的不同在于它的库存， 它的库存是**动态变化**的，**库存之间会互相影响**。
...
![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-9188da91cb8a.png)
...
![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-bd6401739b1c.png)
...
![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-bc0e50744bde.png)
...
![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-46c304d54470.png)
...
极端情况下，一些热门城市（东京很热），中间一些站点可能比较火爆，那两端的人是不是永远买不到票。从业务角度出发 很简单预留票 。放票的时候就放一部分，可以限定只能买长途。短途超过范围不卖了。
...
![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-0c5889fcef90.png)
...
![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-fd658e5751e3.png)

![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-16e982db3334.png)

相比查询的流量，扣减库存的流量是完全可以承载的。后台管理**系统发放票**

![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-74f6e46afd48.png)

由Cannal监听DB变更日志**同步数据**到GemFire即可。

![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-1835033e6f50.png)

![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-34852d0830ff.png)
...
- 早期的 12306 是通过整点去抢票，整点就会产生非常高的流量峰值，对系统造成非常大的压力，后面采取了分时段售票，比如**提前15 天预售**，将抢票的压力按照时间区间分散开，大大减低了峰值。
- 避免黄牛刷光票池的改签和退票，增加**候补车票，**优先异步队列卖给候补登记的人，不会回到票池公开出售。**购票的队列**很多种，同一个票30分钟，10分钟，秒出票
...
![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-9275c0b541dd.png)
...
![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-48629c582df8.png)

2013 年起，铁道部为了应对**黄牛抢票**，以及各类抢票软件和插件，升级了购票验证码系统。

在 12306 官方网站上，从购票到付款，都需要输入验证码。从最开始的字母数字验证码，再到后来升级后的图形验证码，成为了一道 “难过” 的关卡。

![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-42f59d49928b.png)

![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-43554f236d61.png)

各类**奇葩验证码**出现在了 12306 上。

到了 2015 年底，根据有关网站统计，12306 上的图形验证码多达接近 600 种。再经过排列组合，总共有多达 **300000** 种。一次性输入准确的比例仅仅是 8%，两次输入准确比例 27%，三次以上输入准确的比例才勉强超过 60%，如果一次性输入成功的平均用时为 5 秒的话，按照热门车票 “秒光” 的情况计算，每输错一次验证码，就意味着**当次购票成功率下降 80% 左右**。误杀概率很大

上面的候补抢票和验证码机制，主要是为了对抗黄牛。

道高一尺，**魔高一丈。黄牛也在不断进步**。

![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-2a95b3c8b39c.png)

比如你是一个搞黑灰产的人，你可以雇佣一批大学生，去做图形验证码识别。简单验证码还是机器去执行。

那这样的话其实还是没办法防止，但是这样增加了灰产的成本，毕竟人工比机器成本高，从一定程度上还是能够降低灰产的流量占比。

12306 之所以能够使用如此变态的验证码机制的大前提是：**没有把用户体验放在首位**。**必然存在误杀。实则防御黄牛屯票的效果微乎其微。**

好比楼下的一家餐厅，正常情况满足小区进食，排排队还是有机会吃上一口热乎的。但是有人预定了整个市区的饭票，然后坐等你们来排队到我这来买，你说能不饿死人吗？

![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-2f0105ffe0fd.png)

12306 比较特殊，市面上几乎不存在竞争对手，而且火车票对于逢年过节，旅行返乡的人来说，几乎可以等同于必需品。

在这种情况下，12306 不必将用户体验放在首位，对于 12306 来说是可以牺牲部分用户体验来换取系统稳定性。实则要防黄牛，直接封禁第三方购票，购票必须人脸识别即可。

### 动态云服务和查余票/购票分离

- 春运峰值和平时查询流量相差悬殊，查询余票和购买下单分离，采用动态云服务弹性节点插拔。思考为什么要提前将内存几十T打满
...
![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-d39163b3837c.png)

![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-8ccecd4fc0e5.png)
...
![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-35ec342baa54.png)
...
官网VS ：[https://redis.io/comparisons/redis-vs-gemfire/](https://redis.io/comparisons/redis-vs-gemfire/)

Redis 是开源的缓存解决方案，而 GemFire 是商用的，我们在互联网项目中为什么使用 Redis 比较多呢，很大原因就是因为 Redis 是开源的，不要钱。

GemFire，不仅性能强、高可用，而且 GemFire 还会提供一系列的解决方案。

Redis 主要用作缓存存储，而当时 （12 年左右） 12306 最大的瓶颈主要是在 IO 上面。

为什么 12306 最大的瓶颈会在 IO 上面呢？这跟 12306 使用读扩散有关。

读扩散和写扩散常参看面试突击宝典：[千万级实时聊天系统如何设计](https://www.yuque.com/tulingzhouyu/db22bv/rb7gfu08kchy88sx)。

扣减库存**数据库选用的是 Sybase**（收费，关系型数据库）

**数据同步：**

首先通过后台管理系统根据时间车次座位**放票，**更新数据库增加余票，Canal监听bin-log变更，再**异步更新数据到NoSQL缓存** 和 本地Cache多级缓存。前面讲过12306会根据往年压测，提前扩容缓存尽量打满。同步余票到缓存之后，查询余票和扣减直接操作缓存数据。

查询余票服务 和 下单扣减**服务分流**处理。下单抢购完成直接扣减缓存，为确保顺序性可采用分布式锁或者**排队消费**。更多实现过程和源码可参看面试突击宝典：[如何设计一个秒杀系统](https://www.yuque.com/tulingzhouyu/db22bv/zmh8bxm30a2a81p2#nY8CA) 中的扣减库存极限优化部分。缓存扣减同时发送**异步消息批量更新DB**，再交由消息队列同步更新情况**汇总给数据中台**。数据中台可协调余票预分配以及库存预警重分配等。

由于数据量较大，最终汇总DB的数据除了sharding分片读写分离之外，还需要做好ES迁移和大数据相关分析性数据库等备份。这样方便快速查询统计
...
![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-0d5e3239dede.png)
...
理论上来说bitmaps实际通过string类型来存储最大长度可512MB*8位/字节，但实际上我们为了避免大Key影响性能应该将车次按车厢划分（软卧-软座-硬座座位数是36-60-118）。这里为了演示，假定一个站点先发售10张，直接一个k-v搞定

Key 设计: 车次_城市_车厢等（几等座车厢已区分）* *

比如： G1_110000_08

value设计： bitmaps标识可买状态0，已售状态1.

![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-0e3703698b64.png)

当乘客想买 北京南-南京南 时，只需判断 北京南，济南西，南京南三个数组中进行或（||）的操作 即可得到库存余票情况。

![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-50a7c6861917.png)

接下来即可选择某个座位下单购买，

代码层需要将这三个站点的某个坐次位都set key offset value 为1

![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-0ecdc793805a.png)

接下来 更多乘客组合 短中全程票购买之后，可能会是如下结果：

![image](/面试题/项目设计场景题/0794-12306-ticket-grabbing-system-design/img-166ab4dedba5.png)
...
而且运营部来进行设计，首先考虑的肯定是要**盈利**，远途票价比较贵，因此比较倾向于远途的旅客，营业部根据具体的实际情况以及盈利情况来定一下各个区间预留多少票，给每个车站区间都留有一些余票，那么就不会因为某一个区间非常火爆，而导致其他乘客买不到长途的票了。
