...
这个站用来沉淀我在工作和自学里真正啃过的东西：**.NET / Java 源码调试**、**大数据与 Hadoop**、**云原生（Docker / K8s）**、**消息中间件**、**Java 并发编程**、**软件架构（DDD）**、**性能调优（JVM / Tomcat）**、**数据库（MySQL）**、**分布式（Seata）**、**Windows 权限与环境**、**AI 工程化与自动化**，以及一点用 B1 词汇量读论文的英语笔记。
