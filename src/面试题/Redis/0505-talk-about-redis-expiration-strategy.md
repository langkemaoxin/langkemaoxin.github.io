...
title: "说说Redis的过期策略"
sidebarGroup: "Redis"
shortTitle: "说说Redis的过期策略"
order: 505
date: 2025-12-30
category: "面试题"
tag:
  - "面试题"
description: "简答过期策略主要有三种：惰性删除、定期删除和定期淘汰。惰性删除： 惰性删除是Redis默认的过期键删除策略。当客户端尝试访问一个已过期的键时，Redis会立即将该键删除，并返回空值。这种策略的优点是删除操作是在需要时进行，减少了不必要的删除"
article: false
---

> 来源：[说说Redis的过期策略](https://www.yuque.com/tulingzhouyu/db22bv/poc1fls1v38yrxnq)
...
![image](/面试题/Redis/0505-talk-about-redis-expiration-strategy/img-97c95b9b71cf.png)
...
![image](/面试题/Redis/0505-talk-about-redis-expiration-strategy/img-974f86a47780.png)
...
使用 DEL 命令删除体积较大的键， 又或者在使用FLUSHDB 和 FLUSHALL 删除包含大量键的数据库时，造成redis阻塞的情况；另外redis在清理过期数据和淘汰内存超限的数据时，如果碰巧撞到了大体积的键也会造成服务器阻塞。
...
![image](/面试题/Redis/0505-talk-about-redis-expiration-strategy/img-f5a1d31cc463.png)

1、slave-lazy-flush从库接受完 rdb 文件后的 flush 操作

2、lazyfree-lazy-eviction内存达到 maxmemory 时进行淘汰

3、lazyfree-lazy-expirekey 过期删除
