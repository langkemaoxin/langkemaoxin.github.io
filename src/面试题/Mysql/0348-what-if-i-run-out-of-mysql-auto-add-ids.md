---
title: "MySQL自增id用完怎么办？"
sidebarGroup: "Mysql"
shortTitle: "MySQL自增id用完怎么办？"
order: 348
date: 2025-12-30
category: "面试题"
tag:
  - "面试题"
description: "MySQL 里有很多自增的 id，每个自增 id 都是定义了初始值，然后不停地往上加步长。虽然自然数是没有上限的，但是在计算机里，只要定义了表示这个数的字节长度，那它就有上限。比如，无符号整型 (unsigned int) 是 4 个字节，"
article: false
---

> 来源：[MySQL自增id用完怎么办？](https://www.yuque.com/tulingzhouyu/db22bv/te3xwgo1cr4ostea)

MySQL 里有很多自增的 id，每个自增 id 都是定义了初始值，然后不停地往上加步长。虽然自然数是没有上限的，但是在计算机里，只要定义了表示这个数的字节长度，那它就有上限。比如，无符号整型 (unsigned int) 是 4 个字节，上限就是2的32次方。
...
2的32次方不是一个特别大的数，对于一个频繁插入删除数据的表来说，是可能会被用完的。因此在建表的时候你需要考察你的表是否有可能达到这个上限，如果有可能，就应该创建成 8 个字节的 bigint unsigned。
...
![image](/面试题/Mysql/0348-what-if-i-run-out-of-mysql-auto-add-ids/img-a897f3983ed9.png)
...
![image](/面试题/Mysql/0348-what-if-i-run-out-of-mysql-auto-add-ids/img-c3947a9c3bb4.png)
...
![image](/面试题/Mysql/0348-what-if-i-run-out-of-mysql-auto-add-ids/img-b2c8b2b54ff6.png)
...
当然，在 MySQL 里还有别的自增 id，比如 x_id、thread_id、table_id等等，如果用完了又会发生什么呢？大家可以一起在评论区咱们一起聊聊。下期再会~
