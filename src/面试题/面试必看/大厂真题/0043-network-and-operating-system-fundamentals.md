...
title: "网络和操作系统基础篇"
sidebarGroup: "大厂真题"
shortTitle: "网络和操作系统基础篇"
order: 43
date: 2026-03-09
category: "面试题"
tag:
  - "面试题"
description: "网络基础TCP三次握手三次握手过程：客户端——发送带有SYN标志的数据包——服务端一次握手Client进入syn_sent状态；服务端——发送带有SYN/ACK标志的数据包——客户端二次握手服务端进入syn_rcvd；客户端——发送带有AC"
article: false
---

> 来源：[网络和操作系统基础篇](https://www.yuque.com/tulingzhouyu/db22bv/pngo33i92ss0uz3s)
...
3、避免已失效的连接请求干扰 ：在三次握手过程中，如果客户端发出连接请求后意外宕机或网络出现故障等情况，当服务器收到这个错误的连接请求并回复 SYN+ACK 后，由于客户端无法完成第三次握手发送 ACK，服务器在经过一段时间后会自动放弃这个连接请求。如果采用二次握手，服务器可能会一直等待客户端的响应，从而浪费资源。
...
因为需要确保客户端与服务端的数据能够完成传输。详细原因：

- **可靠地关闭双方的发送和接收功能**

- 在 TCP 连接中，每一端都有发送和接收数据的能力，而四次挥手可以确保双方都能正确地关闭自己的发送功能，同时接收到来自对方的数据结束通知。当一方完成数据发送任务后，需要向对方发送一个连接释放报文段（FIN），表示自己已经没有数据要发送了。接收方收到这个 FIN 后，会发送一个确认报文（ACK），但此时接收方可能还有一些数据没有发送完成，所以它不会立即关闭连接，而是会等到自己的数据也全部发送完毕后再向对方发送自己的连接释放报文段（FIN）。发送方收到这个 FIN 后，再发送一个确认报文（ACK），至此，四次挥手完成，连接才完全释放。

- **确保数据传输的完整性**

- 四次挥手过程中，每一次的报文都有其特定的作用，尤其是确认对方的连接释放请求。通过这种方式，可以确保双方在关闭连接前都已收到了对方发送的所有数据。如果像三次握手一样减少到三次挥手，可能会出现一方已经关闭了连接，而另一方的数据还未发送完成的情况，导致数据丢失。

- **避免连接释放过程出现混乱**

- 四次挥手的方式使得连接释放过程更加有序。如果减少到三次挥手，在一方发送 FIN 后，另一方可能在没有正确处理完自己的数据发送和接收的情况下就关闭了连接，从而导致连接释放出现混乱，甚至可能出现连接无法正常关闭的情况。
...
![image](/面试题/面试必看/大厂真题/0043-network-and-operating-system-fundamentals/img-01694d43b27b.png)
...
![image](/面试题/面试必看/大厂真题/0043-network-and-operating-system-fundamentals/img-0bf90fce768c.png)
...
![image](/面试题/面试必看/大厂真题/0043-network-and-operating-system-fundamentals/img-c0b180b83100.png)
...
![image](/面试题/面试必看/大厂真题/0043-network-and-operating-system-fundamentals/img-4f5697ea41c7.png)
...
![image](/面试题/面试必看/大厂真题/0043-network-and-operating-system-fundamentals/img-7d7b8936165b.png)
...
![image](/面试题/面试必看/大厂真题/0043-network-and-operating-system-fundamentals/img-f05b3fddf35e.png)
...
![image](/面试题/面试必看/大厂真题/0043-network-and-operating-system-fundamentals/img-2e30955def7d.png)

**get和Post区别：**

![image](/面试题/面试必看/大厂真题/0043-network-and-operating-system-fundamentals/img-deb8340c7422.png)
...
![image](/面试题/面试必看/大厂真题/0043-network-and-operating-system-fundamentals/img-b403fb6e6a03.png)
