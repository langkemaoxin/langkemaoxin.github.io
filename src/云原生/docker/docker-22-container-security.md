...
> **Docker 系列 · 第 22/24 篇**
> 上一篇：[《Docker Daemon 与 runtime——一条 docker run 经过了谁的手》](/云原生/docker/docker-21-daemon-runtime) · 下一篇：[《构建进阶——同一个镜像从 1.44GB 滚到 20MB》](/云原生/docker/docker-23-build-advanced)
...
第 ① 道前面几篇已经拆完了，第 ④ 道留给第 24 篇。本篇主线是第 ② 道「容器配置」——同一容器一路降权。本机 daemon 实际启用了哪些安全机制，`docker info` 直接给出：
...
两行信息：`seccomp` + `Profile: builtin` 说明默认的系统调用白名单已经挂上了（雪球 7 展开）；`cgroupns` 是 cgroup 命名空间（[第 20 篇](/云原生/docker/docker-20-cgroups/)）。注意这里**没列 `apparmor`**——WSL 内核没启用它，雪球 8 再说。防线清点完毕，下一球把最危险的那种容器跑起来。
...
Compose 写法（[第 13 篇](/云原生/docker/docker-13-compose)字段表的落地）：
...
| daemon socket 保护 | 不随意 `-v /var/run/docker.sock` 进容器 | 第 24 篇展开 |
...
| [第 18 篇](/云原生/docker/docker-18-namespace/) / [第 20 篇](/云原生/docker/docker-20-cgroups/) | 雪球 1：第 ① 道防线（命名空间/cgroups） |
| [第 13 篇](/云原生/docker/docker-13-compose) Compose | 雪球 5：`cap_drop` / `cap_add` 的 YAML 写法 |
| [第 9 篇](/云原生/docker/docker-09-dockerfile/) Dockerfile | 雪球 6：`USER` 固化非 root |
| [第 10 篇](/云原生/docker/docker-10-harbor/) Harbor | 雪球 11：内置 Trivy 扫描 |
| [第 23 篇](/云原生/docker/docker-23-build-advanced/) 构建进阶 | 雪球 11：SBOM/provenance；也是下一篇 |
| [第 24 篇](/云原生/docker/docker-24-daemon-ops/) daemon 运维 | 雪球 1/10：第 ④ 道防线（socket/TLS/rootless） |
...
下一篇：[《构建进阶——同一个镜像从 1.44GB 滚到 20MB》](/云原生/docker/docker-23-build-advanced/)。
