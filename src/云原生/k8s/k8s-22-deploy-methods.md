...
> **Kubernetes 系列 · 第 22/30 篇**  
> 上一篇：[《生产集群部署——kubeadm 从零到高可用》](/云原生/k8s/k8s-21-deploy-kubeadm-ha)  
> 下一篇：[《国产化 OS 与容器运行时——OpenEuler、麒麟、CRI-O 与 iSula》](/云原生/k8s/k8s-23-os-runtimes)

---

## 开头：kubeadm 之外，生产上还有五条路

[上一篇](/云原生/k8s/k8s-21-deploy-kubeadm-ha)我们用 kubeadm 把一个高可用集群从零拉了起来——这是官方默认路径。但真实生产环境里，你还会遇到各式各样的「非默认」需求：
...
| kubeadm（[上篇](/云原生/k8s/k8s-21-deploy-kubeadm-ha)） | 控制面静态 Pod + worker systemd | 中 | 中，细节被封装但可覆盖 | 官方标准路径，绝大多数生产集群 |
...
> ⚠️ 注意后两行：它们是 **kubeadm 单机版**（无 HA、无外部负载均衡），属于[第 21 篇](/云原生/k8s/k8s-21-deploy-kubeadm-ha)的范畴，列在这里只是说明「运行时替换」在 kubeadm 路径下只需改 `--cri-socket` 一个参数，而在二进制路径下要自己装运行时、改 kubelet 的 `--container-runtime-endpoint`。
...
| 一般生产集群，要官方支持路径 | kubeadm（[第 21 篇](/云原生/k8s/k8s-21-deploy-kubeadm-ha)） | 文档最全、生态默认 |
