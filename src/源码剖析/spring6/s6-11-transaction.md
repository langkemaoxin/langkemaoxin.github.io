...
tag:
  - "Spring 6"
  - "源码"
description: "11.声明式事务源码"
---

> 来源：[11.声明式事务源码](https://www.yuque.com/geren-t8lyq/ru879g/otylxldu7t712746)

在线笔记：[https://www.yuque.com/geren-t8lyq/ru879g/otylxldu7t712746?singleDoc#](https://www.yuque.com/geren-t8lyq/ru879g/otylxldu7t712746?singleDoc#) 《9.声明式事务源码》

课上事务流程图：

[https://www.processon.com/view/link/68c27bf5192e3709bb4efbf1?cid=68ba40bc8e2d826854f6940b](https://www.processon.com/view/link/68c27bf5192e3709bb4efbf1?cid=68ba40bc8e2d826854f6940b)

完整流程图：

事务解析：[https://www.processon.com/view/link/5f4f4c195653bb0c71e5f9f4](https://www.processon.com/view/link/5f4f4c195653bb0c71e5f9f4)

事务创建代理：[https://www.processon.com/view/link/5f507c7407912902cf700145](https://www.processon.com/view/link/5f507c7407912902cf700145)

事务调用：[https://www.processon.com/view/link/5f50d4c75653bb53ea8df95a](https://www.processon.com/view/link/5f50d4c75653bb53ea8df95a)
...
![image](/源码剖析/spring6/s6-11-transaction/img-001.png)
...
简直不要太面熟好不好，属性和@EnableAspectJAutoProxy注解一个套路。不同之处只在于@Import导入器导入的这个类，不同的是：它导入的是个ImportSelector
...
AdviceModeImportSelector目前所知的三个子类是：AsyncConfigurationSelector、TransactionManagementConfigurationSelector、CachingConfigurationSelector。由此可见后面还会着重分析的Spring的缓存体系@EnableCaching，和异步@EnableAsync模式也是和这个极其类似的~~~
...
这一步最重要的就是向Spring容器注入了一个自动代理创建器：org.springframework.aop.config.internalAutoProxyCreator，这里有个小细节注意一下，由于AOP和事务注册的都是名字为org.springframework.aop.config.internalAutoProxyCreator  的BeanPostProcessor，但是只会保留一个，AOP的会覆盖事务的， 因为AOP优先级更大

![image](/源码剖析/spring6/s6-11-transaction/img-002.png)
...
// 它是一个AbstractBeanFactoryPointcutAdvisor ，关于这个Advisor 请参阅之前aop讲解~~~
...
[https://www.processon.com/view/link/5f4f4c195653bb0c71e5f9f4](https://www.processon.com/view/link/5f4f4c195653bb0c71e5f9f4)
...
![image](/源码剖析/spring6/s6-11-transaction/img-003.png)

显示的调用getBean方法方法创建我们的BeanFactoryTransactionAttributeSourceAdvisor返回去

![image](/源码剖析/spring6/s6-11-transaction/img-004.png)

## 创建动态代理

[https://www.processon.com/view/link/5f507c7407912902cf700145](https://www.processon.com/view/link/5f507c7407912902cf700145)
...
这里获取到的时候 通过@Import 的ImportSelect 注册的配置类ProxyTransactionManagementConfiguration 中设置的**AnnotationTransactionAttributeSource**：它是基于注解驱动的事务管理的事务属性源，和@Transaction相关，也是现在使用得最最多的方式。

它的基本作用为：它遇上比如@Transaction标注的方法时，此类会分析此事务注解，最终组织形成一个TransactionAttribute供随后的调用。

当然还有其他的类型 ，稍微举几个例：

![image](/源码剖析/spring6/s6-11-transaction/img-005.png)
...
![image](/源码剖析/spring6/s6-11-transaction/img-006.png)
...
![image](/源码剖析/spring6/s6-11-transaction/img-007.png)
...
![image](/源码剖析/spring6/s6-11-transaction/img-008.png)

![image](/源码剖析/spring6/s6-11-transaction/img-009.png)

最终会解析成TransactionAttribute

![image](/源码剖析/spring6/s6-11-transaction/img-010.png)

OK,解析完成！！！

记录当前需要执行事务的方法名，记录到descriptor   方便后续调用时判断

![image](/源码剖析/spring6/s6-11-transaction/img-011.png)
...
[https://www.processon.com/view/link/5f50d4c75653bb53ea8df95a](https://www.processon.com/view/link/5f50d4c75653bb53ea8df95a)

调用开始和AOP是一样的，这里省略之前的代码：

![image](/源码剖析/spring6/s6-11-transaction/img-012.png)

最终调用org.springframework.transaction.interceptor.TransactionAspectSupport#invokeWithinTransaction
...
![image](/源码剖析/spring6/s6-11-transaction/img-013.png)

**关键点：**

前面4行都是获取一些基本信息

![image](/源码剖析/spring6/s6-11-transaction/img-014.png)
...
![image](/源码剖析/spring6/s6-11-transaction/img-015.png)
...
![image](/源码剖析/spring6/s6-11-transaction/img-016.png)
...
![image](/源码剖析/spring6/s6-11-transaction/img-017.png)
...
- doGetTransaction  将能获得ConnectionHolder，因为顶层事务在开启事务时已经存储。  已经存在事务意味着什么不用我说了吧

`  if (isExistingTransaction(transaction)) {`

成立！因为事务ConnectionHolder已经存在  并且 已经激活（在doBegin中激活的）。  执行嵌套事务handleExistingTransaction
...
![image](/源码剖析/spring6/s6-11-transaction/img-018.png)
...
![image](/源码剖析/spring6/s6-11-transaction/img-019.png)

所以最终返回一个DefaultTransactionStatus， 后续回滚、提交 都可以根据改对象进行控制。  回滚提交逻辑比较简单不在这里重复了

## 事务执行监听器

事务执行监听器是 Spring 6.1 新增的功能，主要用于**事务监控**和**统计或日志记录，**事务监听器提供了六个关键的回调方法：

### 事务开始阶段

- `beforeBegin(TransactionExecution transaction)` - 事务开始前的回调
- `afterBegin(TransactionExecution transaction, Throwable beginFailure)` - 事务开始后的回调

### 事务提交阶段

- `beforeCommit(TransactionExecution transaction)` - 事务提交前的回调
- `afterCommit(TransactionExecution transaction, Throwable commitFailure)` - 事务提交后的回调

### 事务回滚阶段

- `beforeRollback(TransactionExecution transaction)` - 事务回滚前的回调
- `afterRollback(TransactionExecution transaction, Throwable rollbackFailure)` - 事务回滚后的回调
...
它与事务事件监听器（`@TransactionalEventListener`）不同，后者是基于应用事件的机制，而事务执行监听器直接集成在事务管理器的生命周期中，就是一种钩子方法回调， 而事件监听用到了订阅发布设计模式。
