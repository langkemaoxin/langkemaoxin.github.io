...
第 1～8 讲的模型（账户 → SID → 令牌 → 组 → ACE）仍然成立。  
缺的是：**让很多台机器共用同一本「人/组账本」**，而不是每台各写一本。

> 本讲严格按西蒙节奏：先发明「公共账本 / 答账的服务器 / 电脑挂上账本」，  
...
（生产环境还有第二台域控、备份、时间同步等——本讲不展开，以免抢走「公共账」这一条主线。）
...
| 公司统一锁屏/软件策略？ | 每台手调 | 可从域下发（**组策略**；本讲只点到「能统一管」，不展开） |
...
工具、脚本、甚至第 3 讲的查找，背后常常通过一套叫 **LDAP** 的约定去读写目录树。  
本讲不背报文；你只要知道：
...
| ACE / 令牌真正比对 | **objectSid**（第 2 讲的 SID） |
...
场景：共享盘不想对 30 个人各写一条 ACE → 建组，人进组，ACE 只写组（第 8、9 讲）。
...
「作用域」影响能嵌套/授权到哪——选错会出现「人进了组却套不上某资源」；细表见官方，本讲不背。  
...
**下一讲才需要：** 访问 `\\服务器\共享` 时，对方如何在**不拿到你密码**的前提下相信「你是谁」——域里常用的「盖章 / 票据」机制。
