...
本篇用 **Docker Compose** 在本机真实搭一个三节点集群（**RabbitMQ 4.3.4 / Erlang 27.3.4.16**），所有命令输出都是实跑结果，讲透 **消息冗余、网络分区、集群运维、前端高可用** 四块。
...
**集群（cluster）就是把多个 RabbitMQ 节点组成一个逻辑 Broker**：客户端连任意一个节点，都能看到相同的 vhost、user、Exchange、Queue **定义**，就像连的是同一台机器。单机其实也是一个「单节点集群」。

![Admin Cluster 页面](/中间件/rabbitmq/15/p08-01.png)

（管理控制台 Admin → Cluster 可查看集群名，默认 `rabbit@hostname`。）

### 最容易误解的一点：集群 ≠ 消息自动冗余

很多人以为「组了集群，消息就会自动存多份」——**这是 RabbitMQ 最大的认知误区**。准确的说法来自官方：

> 📖 **官方原文**（[Clustering Guide](https://www.rabbitmq.com/docs/clustering)）：
...
![同一集群里：Classic 队列消息只存一份，Quorum 队列消息多副本](/中间件/rabbitmq/15/p08-02.png)

> 🔑 **一句话纠偏**：老教程爱讲「普通集群 / 镜像集群」两种模式，那其实是把**集群**和**消息复制**两个正交的概念搅在一起说了。真相是——**集群就一种**，差别只在你往里放什么队列。本篇讲的「高可用」，本质就是「集群 + Quorum 这种会复制的队列」。

---

## 二、用 Docker 搭一个三节点集群（实测）

> 📖 **官方文档**：[Clustering Guide](https://www.rabbitmq.com/docs/clustering)

本节所有命令都在本机真实执行过，环境是 WSL2 Ubuntu-22.04 + Docker（Server 29.1.3），镜像 `rabbitmq:4.3-management`。

### 2.1 三个前提

不论怎么部署，组成集群都绕不开三件事——Docker 把它们都简化了：

**① 节点名要唯一且固定**。节点名 = `前缀@hostname`（前缀默认 `rabbit`），集群节点用它互相寻址。官方原文：

> 📖 **官方原文**：
> "In a cluster, nodes identify and contact each other using node names. This means that the hostname part of every node name must resolve."

手动部署得改 hostname；Docker 用 `hostname:` 字段固定。官方同时警告 hostname 变化会让数据目录对不上、消息「凭空消失」：

> 📖 **官方原文**：
> "by default RabbitMQ names the database directory using the current hostname of the system. If the hostname changes, a new empty database is created. To avoid data loss it's crucial to set up a fixed and resolvable hostname."

**② Erlang Cookie 必须一致**。它是节点间互信的共享密钥，不一致直接拒绝通信：

> 📖 **官方原文**：
> "For two nodes to be able to communicate they must have the same shared secret called the Erlang cookie. ... Every cluster node must have the same cookie."

手动部署要拷 `/var/lib/rabbitmq/.erlang.cookie` 并 `chmod 600`，还容易踩「挂卷后属主变 root、rabbitmq 用户（UID 999）读不到」的容器坑。Docker 镜像专门提供了环境变量来绕开它：

> 📖 **官方原文**：
> "Docker community RabbitMQ image uses `RABBITMQ_ERLANG_COOKIE` environment variable value to populate the cookie file. ... every RabbitMQ node container in a cluster uses the same value."
...
### 2.2 docker-compose.yml（实测用的就是这份）
...
> 🔑 三要点：① `hostname` 固定（否则容器重建数据目录对不上）；② `RABBITMQ_ERLANG_COOKIE` 三节点一致；③ 三者在同一 Compose 网络（默认就是），才能用 service 名互访。`restart: unless-stopped` 很关键——见 2.5 末尾的 WSL 实测坑。
...
> 📖 **关于 4.1 的简化**（[Clustering Guide](https://www.rabbitmq.com/docs/clustering)）：
> "Starting with RabbitMQ 4.1.0, nodes no longer need to be stopped and reset before joining another node. `rabbitmqctl join_cluster` performs the necessary preparations."

> 4.3 不再需要先 `stop_app` / `reset` / `start_app`，一行 `join_cluster` 即可。但 join 是**破坏性操作**（官方：「a node cannot be made a member of another cluster and keep its existing data at the same time」），别拿存了数据的节点去 join。

### 2.4 磁盘节点（disc）vs 内存节点（ram）

`join_cluster` 有个 `--ram` 参数，决定节点的**类型**。先讲清概念，再说为什么 4.3 基本用不上。

**① 节点类型只管「元数据」怎么存，不管「消息」怎么存**——这是最容易混的点。节点是 disc 还是 ram，影响的只是 vhost、user、Exchange、Queue **定义**这些元数据放哪；**队列里的消息该落盘照样落盘，跟节点类型无关**：

> 📖 **官方原文**（[`rabbitmqctl` 手册](https://www.rabbitmq.com/docs/man/rabbitmqctl.8)）：
> "RAM nodes ... replicate data in RAM only, **with the exception of queue contents which can reside on disk if persistent or too large for memory**. RAM nodes are primarily for scalability..."
...
> 📖 **官方原文**（[Production Checklist](https://www.rabbitmq.com/docs/production-checklist)）：
> "Modern RabbitMQ features like Khepri, quorum queues, and streams require durable storage... **Transient entities and RAM node support will be removed in RabbitMQ 4.x.**"

**③ 实测验证**：本集群三个节点 join 时**都没加 `--ram`**（默认 disc），`cluster_status` 里三者全部列在 **Disk Nodes**：
...
4.3 起 Khepri 成为唯一元数据存储，Raft 复制、每个节点都落盘，模型上根本没有「只放内存的元数据节点」，所有节点实质都是 disc。新集群直接忽略 `--ram`。
...
Listeners
  ...
  Node: rabbit@rabbit1, interface: [::], port: 15692, protocol: http,       purpose: Prometheus exporter API
...
浏览器打开 `http://localhost:15672`（默认 `guest/guest`）也能看到三个节点。生产建议 **奇数节点**（3、5、7）：

> 📖 **官方原文**（[Clustering Guide](https://www.rabbitmq.com/docs/clustering)）：
> "Two node clusters are highly recommended against since it's impossible for cluster nodes to identify a majority and form a consensus in case of connectivity loss. ... four or six node clusters would have the same availability characteristics as three and five node clusters."
...
Quorum / Stream 队列都有一个 **leader 节点**，其余是 follower。可用 `x-queue-leader-locator` 控制 leader 落点：

> 📖 **官方原文**（[Clustering Guide](https://www.rabbitmq.com/docs/clustering)）：
> - "`client-local`, the default, will always pick the node the client is connected to"
> - "`balanced`, which takes into account the number of queues/leaders already running on each node"
>
> 配置：`queue_leader_locator = balanced`
...
> 本实测里 `orders` 的 leader 落在了 `rabbit@rabbit1`（声明命令是在 rabbit1 上发的，即 `client-local` 默认行为）。生产建议全局设 `balanced`，避免新队列挤在一个节点上。
...
> 📖 **官方原文**（[Partitions](https://www.rabbitmq.com/docs/partitions)）：
> "This guide was significantly reworked for RabbitMQ `4.3.0`... Starting with `4.3.0`, several important details have changed:
> - RabbitMQ now supports only one metadata store: the Raft-based Khepri
> - Mnesia was removed
> - **Partition handling strategies, necessary during the Mnesia era, were also removed**"

从 **4.3** 起，元数据存储彻底切到 Khepri（Raft），Mnesia 移除；**3.x ~ 4.2 的四种分区策略（`ignore` / `pause_minority` / `pause_if_all_down` / `autoheal`）全部删除**。Raft 内建了分区安全保证，不再需要配置策略：
...
旧配置项虽能写进 `rabbitmq.conf` 但已**完全失效**：

> 📖 **官方原文**（Deprecated `rabbitmq.conf` Keys）：
> "the following `rabbitmq.conf` keys... are accepted but have **no effect**: `cluster_partition_handling`, `cluster_partition_handling.pause_if_all_down.recover`, `cluster_partition_handling.pause_if_all_down.nodes.$name`. These keys should be removed from the configuration files at the earliest opportunity."
...
新节点 join 的流程同 2.3。但 join 后，**已存在的 Quorum 队列默认不会自动把副本扩到新节点**，需手动加：
...
> 📖 **官方原文**：副本会一起被删——"all quorum queue and stream replicas on the node will be removed, even if that means that queues and streams would temporarily have an even (e.g. two) replicas." 所以下线前务必确认剩余副本**还有 quorum**。
...
> 📖 **官方原文**（drain 做了什么）："When a node is in maintenance mode, it **will not be available for serving client traffic** and will try to transfer as many of its responsibilities as practically possible and safe."（暂停监听、关闭连接、迁移 quorum 队列主副本并退出 Raft 选举、标记维护下线。）
...
集群解决了数据冗余，但客户端仍可能**连到已宕节点**——它不知道该换到哪个地址。解决办法是在集群前加一层负载均衡，让客户端只认**一个固定地址**。
...
> 📖 **官方原文**（关于 LB 误杀空闲长连接 + 心跳保护）："Intermediaries also may terminate 'idle' TCP connections... heartbeats have a side effect of guarding client connections that can go idle... against premature closure by proxies and load balancers. Heartbeat timeouts from 10 to 30 seconds will produce periodic network traffic often enough... to satisfy defaults of most proxy tools." → 客户端开启 AMQP 心跳（10–30s）既防误杀又更快发现死连接。
...
不引入 HAProxy 时，客户端自己也能容灾——**连一组地址，断了自动重连**：

> 📖 **官方原文**（[Upgrading RabbitMQ](https://www.rabbitmq.com/docs/upgrade)）：
> "most client libraries accept a list of endpoints (hostnames or IP addresses) as a connection option. The list of hosts will be used during initial connection as well as connection recovery, if the client supports it."
