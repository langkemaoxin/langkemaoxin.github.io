...
> **Kubernetes 系列 · 第 26/30 篇**  
> 上一篇：[《安全容器运行时——Kata Containers 与 gVisor》](/云原生/k8s/k8s-25-sandbox-runtimes)  
> 下一篇：[《分布式存储方案——Longhorn 与 GlusterFS》](/云原生/k8s/k8s-27-storage-longhorn-glusterfs)

---

## 开头：滚动更新够用吗？

[第 13 篇](/云原生/k8s/k8s-13-release-strategies)里我们用原生 Deployment 跑过蓝绿、金丝雀、滚动与 A/B 测试，[第 17 篇](/云原生/k8s/k8s-17-jenkins-canary)又用 Jenkins + Ingress 注解把灰度做成了流水线。但原生方案有三个绕不过去的坎：
...
上一节的权重只是副本比例——Service 依然按 Endpoints 均衡，`20%` 权重是「5 个副本里 1 个新版本」。要真正按流量百分比切分，需要接入流量管理层。Argo Rollouts 主要集成两类：**Ingress 控制器**（NGINX、ALB）和**服务网格**（Istio、Linkerd、SMI），底层依赖[第 12 篇](/云原生/k8s/k8s-12-ingress-l7)讲的 Ingress 能力。

> ⚠️ NGINX Ingress 走 LoadBalancer 暴露时，裸集群需要先部署 MetalLB 提供 `LoadBalancer` IP。核心动作：kube-proxy 开启 `strictARP: true` 并设 `mode: "ipvs"`，然后 `kubectl apply -f metallb-native.yaml`，再建 `IPAddressPool`（如 `192.168.10.240-192.168.10.250`）和 `L2Advertisement`。这套准备在[第 17 篇](/云原生/k8s/k8s-17-jenkins-canary)的灰度环境里也用过。
...
Metric Providers 原生集成 Prometheus、Wavefront、Kayenta、Web、Kubernetes Jobs 等。接 Prometheus 后，典型玩法是：金丝雀暂停期间跑 AnalysisRun，查询新版本的错误率/延迟（查询能力可参考[第 18 篇](/云原生/k8s/k8s-18-prometheus-hpa)的监控体系），达标自动 `promote`、超标自动 `abort`。
...
[第 17 篇](/云原生/k8s/k8s-17-jenkins-canary)的流水线里，灰度的「改权重 → 观察 → 再改权重」是 Jenkins Stage 里的 `kubectl` 命令，权重计算与推进逻辑全写在 Pipeline 里。两套方案都能替换掉这部分手工逻辑：
...
- 与 [第 17 篇](/云原生/k8s/k8s-17-jenkins-canary)的 Jenkins 流水线组合时，把灰度状态从流水线变量移到集群 CR，Jenkins 只留「构建 + 触发 + 验收放行」。

下一篇转向存储：[《分布式存储方案——Longhorn 与 GlusterFS》](/云原生/k8s/k8s-27-storage-longhorn-glusterfs)。
