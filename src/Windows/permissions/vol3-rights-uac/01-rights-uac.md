...
本讲用**两则小故事**告诉他：世界上还有门上没有写的规矩；以及「我是管理员」为什么还要点同意。  
专有名词都放在故事推导之后。
...
1. `icacls` / 资源管理器安全页，管的是**这个文件夹**上的规则——第 9、10 讲的 Access Check。  
...
- **不要**在本讲练习「如何用备份权利去读 Deny 目录」——那是攻击细节，我们只建立正确地图。
...
顺带一句（呼应第 15 讲）：高权账户（如域管）往往还能**夺所有权再改门上的字**，所以日常别拿域管账号逛网页、翻文件。  
来源：同上 Appendix B。
...
| 它不负责 | 那是谁的事 |
|----------|------------|
| 决定「这个文件夹谁能读」 | 对象权限 / ACE（[第 9～10 讲](../vol1-invent/10-ace-dacl.md)） |
| 决定「准不准备份整盘、当服务登录」 | 用户权利 / privileges（本讲故事一） |
| 网上证明「我是域里的谁」 | 登录 + Kerberos 等（[第 4](../vol1-invent/05-logon-lsa.md)、[16 讲](../vol2-identity/02-kerberos.md)） |
...
第 [5 讲](../vol1-invent/06-access-token.md) 说过：令牌挂在**进程**上，里面至少有「你是谁、你带哪些组、你有哪些特权」。  
...
3. **真正的标准用户账户**（不在 Administrators 里）通常只有一张日常票；要做管理员活，常要另输管理员账号密码（凭据提示），那是另一条路，本讲先抓住「管理员双令牌」即可。
...
小王能保存，只说明：**这一次写入，通过了「当前进程令牌 vs 该文件 ACL」的检查**（以及若走共享还会有两道门，见 [第 10 讲](../vol1-invent/11-access-check.md)）。常见三种故事：

1. **进程已是管理员票**（标题有「管理员:」、强制标签 High）→ 过 ACL 很正常。  
2. **UAC 关着，日常就几乎是管理员票** → 看起来「普通记事本也能改」。  
3. **有人改过 hosts 的 ACL**（给 Users / 某域组写权限）→ **日常票也能写**；这和第 9 讲「门上的字」是一回事，不能反推「我整机最高权」。
...
| 我是最高权限了么？ | 若强制标签已是 **High** 且 Administrators 为完整启用 → **当前进程**是管理员票；账户层「还能不能更高」本讲不展开（如 SYSTEM 等） |
...
- [第 5 讲](../vol1-invent/06-access-token.md)：权限检查看的是**进程令牌**，不是「账户名片上写没写管理员」。  
- [第 16 讲](../vol2-identity/02-kerberos.md)：票据解决**网上如何证明你是谁**；UAC 解决**本机进程揣哪张通行证**。别混成一个词。
...
| 和前几讲 | ACE 仍重要，但不唯一 | [第 5 讲](../vol1-invent/06-access-token.md)「令牌挂在进程上」在这里落地 |
...
系列正文到此。请到 [总图](../appendix/01-map.md) 串线复习，或回 [索引](../00-index.md) 跳讲；更多出处见 [参考](../appendix/05-references.md)。
