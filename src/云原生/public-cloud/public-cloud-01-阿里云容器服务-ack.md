---
title: 阿里云容器服务 ACK
sidebarGroup: 公有云
shortTitle: 01 阿里云容器服务 ACK
order: 1
date: 2026-08-13T00:00:00.000Z
category: 云原生
tag:
  - 公有云
  - 云原生
  - 课程笔记
description: 阿里云容器服务ACK 一、配置专有网络 1.1进入阿里云VPC控制台 1.2开通阿里云VPC服务 1.3创建vpc 二、配置Kubernetes集群 2.1进入阿里云ACK控制台 2.2创建集群 2....
---

> **公有云 · 第 1 篇**
>
> 来源课程笔记整理优化；插图已迁入博客静态目录。

---

# 阿里云容器服务ACK

# 一、配置专有网络

## 1.1进入阿里云VPC控制台

![进入VPC控制台](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/进入VPC控制台.png)

## 1.2开通阿里云VPC服务

![vpc控制台](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/vpc控制台.png)

![vpc开通成功](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/vpc开通成功.png)

## 1.3创建vpc

![创建vpc](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/创建vpc.png)

![配置vpc01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/配置vpc01.png)

![配置vpc02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/配置vpc02.png)

![image-20221012160149721](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/image-20221012160149721.png)

# 二、配置Kubernetes集群

## 2.1进入阿里云ACK控制台

![容器服务ack](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/容器服务ack.png)

![进入ack控制台](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/进入ack控制台.png)

## 2.2创建集群

### 2.2.1进入集群配置界面

![创建集群](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/创建集群.png)

### 2.2.2选择集群配置

![配置k8s01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/配置k8s01.png)

![配置k8s02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/配置k8s02.png)

![配置k8s03](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/配置k8s03.png)

### 2.2.3选择节点池配置

![配置nodepool01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/配置nodepool01.png)

![配置nodepool02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/配置nodepool02.png)

### 2.2.4选择组件配置

![配置组件01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/配置组件01.png)

### 2.2.5确认配置

![创建集群02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/创建集群02-16655626080871.png)

### 2.2.6创建集群

![集群创建中](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/集群创建中.png)

![集群列表](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/集群列表.png)

# 三、阿里云控制台测试集群可用性

## 3.1创建Deployment

![demo01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/demo01.png)

![demo02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/demo02.png)

![demo03](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/demo03.png)

## 3.2创建Service

![svcDemo01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/svcDemo01.png)

![svcDemo02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/svcDemo02-16655630626753.png)

![svcDemo03](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/svcDemo03.png)

## 3.3创建Ingress

![ingressDemo01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/ingressDemo01.png)

![ingressDemo02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/ingressDemo02.png)

![访问01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/访问01.png)

## 3.4本地pc添加hosts(ingress域名未做备案)

同时按住win键+R键进入运行

![修改hosts01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/修改hosts01.png)

![修改hosts02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/修改hosts02.png)

![修改hosts03](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/修改hosts03.png)

![修改hosts04](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/修改hosts04.png)

## 3.5访问应用

![访问02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/访问02.png)

# 四、通过api连接集群，并使用阿里云容器镜像仓库服务进行应用发布

## 4.1 安装kubectl

~~~powershell
[root@kubemsb ~]# curl -LO "https://dl.k8s.io/release/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/amd64/kubectl"

[root@kubemsb ~]# chmod +x kubectl

[root@kubemsb ~]# install -o root -g root -m 0755 kubectl /usr/local/bin/kubectl
      
[root@kubemsb ~]# kubectl version
WARNING: This version information is deprecated and will be replaced with the output from kubectl version --short.  Use --output=yaml|json to get the full version.
Client Version: version.Info{Major:"1", Minor:"25", GitVersion:"v1.25.2", GitCommit:"5835544ca568b757a8ecae5c153f317e5736700e", GitTreeState:"clean", BuildDate:"2022-09-21T14:33:49Z", GoVersion:"go1.19.1", Compiler:"gc", Platform:"linux/amd64"}
Kustomize Version: v4.5.7
Server Version: version.Info{Major:"1", Minor:"24+", GitVersion:"v1.24.6-aliyun.1", GitCommit:"6965b276168c8f1e4a8545c52152838a2f433758", GitTreeState:"clean", BuildDate:"2022-09-22T08:06:05Z", GoVersion:"go1.18.6", Compiler:"gc", Platform:"linux/amd64"}
~~~

## 4.2 下载kubeConfig文件

![连接k8s01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/连接k8s01.png)

![连接k8s02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/连接k8s02.png)

~~~powershell
[root@kubemsb ~]# mkdir /root/.kube
[root@kubemsb ~]# vim /root/.kube/config
apiVersion: v1
clusters:
- cluster:
    server: https://你的集群ip:6443
    certificate-authority-data: 
  name: kubernetes
contexts:
- context:
    cluster: kubernetes
    user: "kubernetes-admin"
  name: kubernetes-admin-c0aa8c47fbfab4b7ebf610d0214bfacd5
current-context: kubernetes-admin-c0aa8c47fbfab4b7ebf610d0214bfacd5
kind: Config
preferences: {}
users:
- name: "kubernetes-admin"
  user:
    client-certificate-data: 
    client-key-data: 
~~~

## 4.3 查看集群信息

~~~powershell
[root@kubemsb ~]# kubectl get nodes --kubeconfig /root/.kube/config
NAME                       STATUS   ROLES    AGE     VERSION
cn-chengdu.192.168.10.43   Ready    <none>   3h37m   v1.24.6-aliyun.1
cn-chengdu.192.168.10.44   Ready    <none>   3h37m   v1.24.6-aliyun.1
cn-chengdu.192.168.10.45   Ready    <none>   3h37m   v1.24.6-aliyun.1
~~~

## 4.4 创建阿里云容器镜像服务

![镜像服务](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/镜像服务.png)

![镜像服务02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/镜像服务02.png)

![镜像服务03](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/镜像服务03.png)

![创建仓库01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/创建仓库01.png)

![创建仓库05](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/创建仓库05.png)

![创建仓库03](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/创建仓库03.png)

![创建仓库04](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/创建仓库04.png)

## 4.5 安装docker

~~~powershell
[root@kubemsb ~]# wget -O /etc/yum.repos.d/docker-ce.repo https://mirrors.aliyun.com/docker-ce/linux/centos/docker-ce.repo
[root@kubemsb ~]# yum -y install docker-ce
[root@kubemsb ~]# systemctl enable docker
[root@kubemsb ~]# systemctl start docker
~~~

## 4.6 上传镜像至服务器

将资源目录下的“apidemo.tar上传至服务器”

## 4.7 导入镜像

~~~powershell
[root@kubemsb ~]# docker load -i apidemo.tar
[root@kubemsb ~]# docker images
REPOSITORY                                         TAG       IMAGE ID       CREATED         SIZE
apidemo                                            v1        cae07723a2c3   7 minutes ago   15.5MB
~~~

## 4.8 登录镜像仓库

~~~powershell
[root@kubemsb ~]# docker login --username=你的用户名 registry.cn-chengdu.aliyuncs.com
~~~

## 4.9 上传镜像至阿里云镜像仓库

~~~powershell
[root@kubemsb ~]# docker tag apidemo:v1 registry.cn-chengdu.aliyuncs.com/kubemsb/apidemo:v1
[root@kubemsb ~]# docker push registry.cn-chengdu.aliyuncs.com/kubemsb/apidemo:v1
~~~

![image-20221012170435435](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/image-20221012170435435.png)

## 4.10在阿里云控制台部署项目

### 4.10.1 部署apidemo_Deployment

![apidemo01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/apidemo01.png)

![apidemo02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/apidemo02.png)

![apidemo03](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/apidemo03.png)

![apidemo04](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/apidemo04.png)

![apidemo05](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/apidemo05.png)

![image-20221012170947145](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/image-20221012170947145.png)

![image-20221012171112503](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/image-20221012171112503.png)

![apidemo08](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/apidemo08.png)

![apidemo09](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/apidemo09.png)

### 4.10.2 部署apidemo_SVC

![apidemosvc01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/apidemosvc01.png)

![apidemosvc02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/apidemosvc02.png)

### 4.10.3 部署apidemo_ingress

![apidemoingress01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/apidemoingress01.png)

![apidemoingress02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/apidemoingress02.png)

![apidemoingress03](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/apidemoingress03.png)

![apidemoingress04](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/apidemoingress04.png)

![apidemoingress05](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/apidemoingress05.png)

## 4.11 访问apidemo服务

### 4.11.1 pc添加hosts

![修改hosts01](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/修改hosts01-16655662413204.png)

![修改hosts02](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/修改hosts02-16655662470795.png)

![修改hosts03](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/修改hosts03-16655662512246.png)

![修改hosts05](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/修改hosts05.png)

### 4.11.2 访问apidemo服务

> 浏览器访问apidemo.kubemsb.com/info

返回如下信息则证明服务部署成功

![image-20221012171836782](/云原生/public-cloud/public-cloud-01-阿里云容器服务-ack/image-20221012171836782.png)

