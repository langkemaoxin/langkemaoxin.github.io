...
> **Kubernetes 系列 · 第 27/30 篇**  
> 上一篇：[《发布进阶——Argo Rollouts 金丝雀与 OpenKruise 原地升级》](/云原生/k8s/k8s-26-advanced-rollout)  
> 下一篇：[《网络进阶——Cilium、Hybridnet、双栈与 Traefik》](/云原生/k8s/k8s-28-advanced-network)
...
Longhorn 自带 UI，需要从集群外访问。裸机集群没有云 LoadBalancer，先准备 **MetalLB**（给 Service 分配外部 IP）与 **Ingress-NGINX**（七层转发），这在[第 12 篇 Ingress](/云原生/k8s/k8s-12-ingress-l7)中有过铺垫。
...
下一篇进入**网络进阶**：Cilium eBPF 数据面、Hybridnet 多栈网络与 Traefik 网关。
