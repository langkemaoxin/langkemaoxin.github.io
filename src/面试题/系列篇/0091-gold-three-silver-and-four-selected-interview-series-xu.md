---
title: "金三银四精选面试题系列-徐庶"
sidebarGroup: "系列篇"
shortTitle: "金三银四精选面试题系列-徐庶"
order: 91
date: 2026-04-13
category: "面试题"
tag:
  - "面试题"
description: "Java中有哪几种方式来创建线程执行任务？薪资：8-12k岗位：初级开发工程师1. 继承Thread类/**  * 作者：徐庶  */ public class XushuThread extends Thread{       publi"
article: false
---

> 来源：[金三银四精选面试题系列-徐庶](https://www.yuque.com/tulingzhouyu/db22bv/gd98aqe6nn6e73qq)
...
![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-ba944aabe685.png)
...
不可获取当前线程是否上锁
可获取当前线程是否上锁isHeldByCurrentThread
...
![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-07228f7d3abd.png)

1. ThreadLocal底层是通过ThreadLocalMap来实现的，每个Thread对象（注意不是ThreadLocal对象）中都存在一个ThreadLocalMap，Map的key为ThreadLocal对象，Map的value为需要缓存的值
![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-a1896edb556b.png)
2. 如果在线程池中使用ThreadLocal会造成内存泄漏，因为当ThreadLocal对象使用完之后，应该要把设置的key，value，也就是Entry对象进行回收，但线程池中的线程不会回收，而线程对象是通过强引用指向ThreadLocalMap，ThreadLocalMap也是通过强引用指向Entry对象，线程不被回收，Entry对象也就不会被回收，从而出现内存泄漏，解决办法是，在使用了ThreadLocal对象之后，手动调用ThreadLocal的remove方法，手动清楚Entry对象

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-1814a1e3a8bb.png)
...
1. 当一个共享变量是共享的， 但是需要每个线程互不影响，相互隔离 ， 就可以使用ThreadLocal
...
![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-4986372e1380.png)

**非公平锁的底层执行流程：**

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-c7986dafa5a8.png)
...
1. **两个或以上线程并发的在同一个对象上进行同步时**，为了避免无用自旋消耗cpu，轻量级锁会升级成重量级锁

1. 自旋锁：自旋锁就是线程在获取锁的过程中，不会去阻塞线程，也就无所谓唤醒线程，**阻塞和唤醒这两个步骤都是需要操作系统去进行的**，比较消耗时间，自旋锁是线程通过CAS获取预期的一个标记，如果没有获取到，则继续循环获取，如果获取到了则表示获取到了锁，这个过程线程一直在运行中，相对而言没有使用太多的操作系统资源，比较轻量。
...
![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-9ec5f3f00de9.png)
...
![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-482273bdeb14.png)

a字段是主键，对应主键索引，bcd三个字段组成一个联合索引，e字段一个索引

### 1. 不符合最左匹配原则

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-69f17d142a66.png)

去掉b=1的条件就不符合最左匹配原则了，导致所有失效

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-81fda71e1c52.png)

### 2. 不正确的Like查询

不用like能走索引：

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-09a303f6382b.png)

正常使用like：

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-5e7dd39e0898.png)

不正确使用like：

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-fcc30dcf674a.png)

### 3. 对索引列进行了计算或使用了函数

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-d71b3ae53100.png)

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-7c7d47de95f1.png)

### 4. 索引列进行了类型转换

e字段的类型是vachar，下面这个sql需要把e字段中的字符转换成数字，会导致索引失效

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-8b2725481215.png)

### 5. <>不等于导致索引失效

b=1可以走索引，b<>1就不能走索引

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-038dea75ee08.png)

### 6. order by导致索引失效

就算利用索引，但是由于是select * 所以需要回表，而且回表成本比较高，所以不会走索引。

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-eecde4dbd977.png)

如果是select b就需要回表了，就会选择走索引

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-eec55da6b228.png)

### 7. 使用or导致索引失效

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-5d1908501150.png)

### 8. select * 导致索引失效

![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-f9e66b466b6e.png)
...
![image](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-8310c6148773.png)
...
![B+树](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-b047e2bfd2f6.webp)

![B树](/面试题/系列篇/0091-gold-three-silver-and-four-selected-interview-series-xu/img-d68a5f85ef8c.png)
