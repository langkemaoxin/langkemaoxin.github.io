...
title: "有哪些分布式锁的实现方案"
sidebarGroup: "分布式"
shortTitle: "有哪些分布式锁的实现方案"
order: 722
date: 2026-06-07
category: "面试题"
tag:
  - "面试题"
description: "资：12-25K岗位：中高级开发工程师一、分布式锁场景一般需要使用分布式锁的场景如下：效率：使用分布式锁可以避免不同节点重复相同的工作，比如避免重复执行定时任务等；正确性：使用分布式锁同样可以避免破坏数据正确性，如果两个节点在同一条数据上面"
article: false
---

> 来源：[有哪些分布式锁的实现方案](https://www.yuque.com/tulingzhouyu/db22bv/hlbwcoo3qblhpcq3)
...
![image](/面试题/分布式/0722-distributed-lock-solutions/img-31b47093c311.png)
...
lock_duration
integer
锁的超时时长，单位可以业务自定义，通常为秒

lock_owner
varchar
锁的持有者，可以是节点或线程的唯一标识，不同可重入粒度的锁有不同的含义
...
![image](/面试题/分布式/0722-distributed-lock-solutions/img-6c6eebee0032.png)
...
![image](/面试题/分布式/0722-distributed-lock-solutions/img-383eaf089478.jpg)
...
![image](/面试题/分布式/0722-distributed-lock-solutions/img-b49ba90a1812.png)

之后，Client1 查找 ParentLock 下面所有的临时顺序节点并排序，判断自己所创建的节点 Lock1 是不是顺序最靠前的一个。如果是第一个节点，则成功获得锁。

![image](/面试题/分布式/0722-distributed-lock-solutions/img-81bd564299db.png)

这时候，如果再有一个客户端 Client2 前来获取锁，则在 ParentLock 下再创建一个临时顺序节点Lock2。

![image](/面试题/分布式/0722-distributed-lock-solutions/img-c42ddfbfa17e.png)
...
![image](/面试题/分布式/0722-distributed-lock-solutions/img-afcb75c1d4ad.png)

这时候，如果又有一个客户端 Client3 前来获取锁，则在ParentLock下载再创建一个临时顺序节点Lock3。

![image](/面试题/分布式/0722-distributed-lock-solutions/img-70d1a4ddb64f.png)
...
![image](/面试题/分布式/0722-distributed-lock-solutions/img-bdc92c5caf05.png)
...
![image](/面试题/分布式/0722-distributed-lock-solutions/img-0c4d73a67044.png)

2.任务执行过程中，客户端崩溃

获得锁的 Client1 在任务执行过程中，如果 Duang 的一声崩溃，则会断开与 Zookeeper 服务端的链接。根据临时节点的特性，相关联的节点 Lock1 会随之自动删除。

![image](/面试题/分布式/0722-distributed-lock-solutions/img-0fdaa1b0c2dc.png)
...
![image](/面试题/分布式/0722-distributed-lock-solutions/img-877d8bcaf5b4.png)

同理，如果 Client2 也因为任务完成或者节点崩溃而删除了节点 Lock2，那么 Client3 就会接到通知。

![image](/面试题/分布式/0722-distributed-lock-solutions/img-c902a7e4b567.png)

最终，Client3 成功得到了锁。

![image](/面试题/分布式/0722-distributed-lock-solutions/img-e8c8f8bdd0d1.png)
