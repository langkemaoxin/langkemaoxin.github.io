...
> **Docker 系列 · 第 21/24 篇**
> 上一篇：[《CGroups 限资源——给同一个容器逐项上枷锁》](/云原生/docker/docker-20-cgroups) · 下一篇：[《容器安全——同一个容器，从 --privileged 全裸滚到最小权限》](/云原生/docker/docker-22-container-security)
...
| **11** | 全景拼图 | 五级链路串讲，与第 19 篇进程实验互相印证 |
| **12** 🧗 | CRI | K8s 为什么弃用 dockershim；Docker 镜像在 K8s 上还能不能跑 |

前置：[第 3 篇](/云原生/docker/docker-03-engine-platform) 已画过组件地图，[第 19 篇](/云原生/docker/docker-19-process-view) 在进程树上见过 shim——本篇把这条链**按职责拆开**。更高/低层 runtime 分类、containerd 单独安装与 `ctr` 实操在附篇 [a01](/云原生/docker/docker-a01-container-runtime-docker)、[a02](/云原生/docker/docker-a02-containerd)，比本篇更深，这里只回链。
...
三条命令各干什么：`docker top <容器名>` 列出容器内进程在宿主机上的 PID（[第 19 篇](/云原生/docker/docker-19-process-view)用过）；`ps -ef` 配合 `grep` 过滤，找名叫 containerd-shim 的进程；`pstree -l -a -A <shim_pid> -p` 以某个 PID 为根把子树画出来，`-p` 连 PID 一起显示，`-A` 表示用 ASCII 字符画树（终端不乱码）。
...
- 最底下 `sh mqbroker ...`（PID 3473）：**容器里真正跑的应用**（这台实验机上是个 RocketMQ broker 启动脚本），它就是容器内的 PID 1——第 19 篇讲过的「同一个进程两套 PID」
...
- 关掉终端、CLI 崩掉，容器照跑
- 把客户端装在另一台机器上、指到这台的 daemon，一样能管容器（`DOCKER_HOST` / `docker context`，远程管理见[第 24 篇](/云原生/docker/docker-24-daemon-ops)）
...
2. **记账**：镜像、网络（[第 11 篇](/云原生/docker/docker-11-network)的 bridge）、卷（[第 12 篇](/云原生/docker/docker-12-data-persistence)）这些「资产」都归它管——`docker ps`、`docker images` 的答案从这来
...
- **文件系统 layers**：每层只存「相对上一层改了什么」，层带 hash，所以不同镜像能共享同一层——[第 17 篇](/云原生/docker/docker-17-unionfs)把一层层叠出 rootfs 的过程做成了实验
...
- `paused` 是运行中的冻结态（用的正是[第 20 篇](/云原生/docker/docker-20-cgroups)子系统表里的 freezer 思路）
...
现在把第 1、4 节的悬念解开：容器进程的父进程是 shim，不是 dockerd；shim 只认 containerd 的 socket，根本不知道 dockerd 的死活；runc 更是容器一起来就退场了。所以 daemon 重启，shim 带着容器原地不动，`docker logs` 也不会断流。生产上更稳的官方开关 `live-restore` 见[第 24 篇](/云原生/docker/docker-24-daemon-ops)。
...
- [第 18 篇](/云原生/docker/docker-18-namespace)的 **Namespace**（pid/net/mnt/uts/ipc/user）——是 runc 设置的
- [第 20 篇](/云原生/docker/docker-20-cgroups)的 **Cgroup** 限额——是 runc 创建 cgroup 并把容器 PID 写进去的
- [第 19 篇](/云原生/docker/docker-19-process-view)「kill 容器内 PID 1 → 容器退出」——signal 发送与 namespace 清理也是 runc 的活
...
再拿[第 19 篇](/云原生/docker/docker-19-process-view)的进程实验来互相印证——两篇看的是同一个东西，一个从进程看，一个从架构看：
...
表里每一行左右都能对上：第 19 篇实测容器内外进程的宿主机 PPID 都是 shim，本篇解释了为什么——runc 干完就退，shim 才是那个留下的爹。
...
- **runc 的出身**：它不是为 Docker 新写的项目，而是 Docker 把自家 libcontainer 捐给 OCI 后的参考实现（第 10 节）；再往前数，血脉是 LXC 那套「Namespace + Cgroup 封装」路线——[第 16 篇](/云原生/docker/docker-16-tech-foundation)的「Docker ≈ LXC + AUFS」、[第 20 篇](/云原生/docker/docker-20-cgroups)的「Cgroup → LXC → Docker」链条讲的就是这段。这也是 runc 子命令风格和 docker 相近的原因（[第 3 篇](/云原生/docker/docker-03-engine-platform)提过）。
...
| [第 16 篇](/云原生/docker/docker-16-tech-foundation) 技术底座 | Namespace / Cgroup / UnionFS 三大件总览 |
| [第 17 篇](/云原生/docker/docker-17-unionfs) UnionFS | 第 6 节镜像 layers 的叠法 |
| [第 18 篇](/云原生/docker/docker-18-namespace) Namespace | 第 10 节：隔离是 runc 设置的 |
| [第 19 篇](/云原生/docker/docker-19-process-view) 进程视角 | 第 1、11 节互相印证 |
| [第 20 篇](/云原生/docker/docker-20-cgroups) Cgroups | 第 10 节：限额是 runc 写入的 |
| [第 22 篇](/云原生/docker/docker-22-container-security) 容器安全 | 下一篇：这条链上的权限怎么降 |
| [第 24 篇](/云原生/docker/docker-24-daemon-ops) Daemon 运维 | daemon.json / live-restore 的落地 |
...
11. **全景（第 11 节）**：五级接力串成一张图，与第 19 篇进程实验一一对应；dockerd 已瘦身。
...
欢迎在评论区留下你的分析。下一篇：[《容器安全——同一个容器，从 --privileged 全裸滚到最小权限》](/云原生/docker/docker-22-container-security)。
