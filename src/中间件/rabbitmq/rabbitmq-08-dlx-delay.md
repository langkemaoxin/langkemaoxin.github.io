...
| 方案 | 精度 | 适用 |
|------|------|------|
| **Redis ZSet / sorted set + 轮询** | 毫秒级（取决于轮询间隔） | 短延迟、高精度场景 |
| **RocketMQ 延迟消息** | 秒级（固定延迟级别）/ 毫秒级（任意延迟 5.x） | 已用 RocketMQ 的项目 |
| **Java DelayQueue / ScheduledExecutor** | 毫秒级（单机） | 进程内延迟、不需要分布式 |
| **定时任务扫表** | 分钟级 | 数据库里有"到期时间"字段，定时任务扫到期记录 |
| **时间轮（Netty HashedWheelTimer）** | 毫秒级 | 大量短延迟任务，进程内 |

> 大多数业务（订单关单、提醒推送）用 TTL + DLX 的秒级精度就够了——30 分钟延迟差个 0.5 秒无所谓。如果真的要毫秒精确，通常是性能敏感场景（限流、超时控制），那种更多用 Redis 或进程内方案，不走 MQ。
