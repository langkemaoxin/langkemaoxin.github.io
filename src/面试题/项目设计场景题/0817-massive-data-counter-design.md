...
shortTitle: "海量数据的计数器要如何设计"
order: 817
date: 2025-12-30
category: "面试题"
tag:
  - "面试题"
description: "刷微博、点赞热搜，如果有抽奖活动，再转发一波，其实就是微博场景下的计数数据，细说起来，它主要有几类：微博的评论数、点赞数、转发数、浏览数、表态数等等；用户的粉丝数、关注数、发布微博数、私信数等等。微博维度的计数代表了这条微博受欢迎的程度，用"
article: false
---

> 来源：[海量数据的计数器要如何设计](https://www.yuque.com/tulingzhouyu/db22bv/br561cfm11x25p0n)
...
![image](/面试题/项目设计场景题/0817-massive-data-counter-design/img-7312b74c9ff7.png)
...
![image](/面试题/项目设计场景题/0817-massive-data-counter-design/img-963013cbe1dd.png)
...
1. 一是原生的Redis在存储Key时是按照字符串类型来存储的，比如一个8字节的Long类型的数据，需要8（sdshdr数据结构长度）+ 19（8字节数字的长度）+1（’\0’）=28个字节，如果我们使用Long类型来存储就只需要8个字节，会节省20个字节的空间；
2. 二是去除了原生Redis中多余的指针，如果要存储一个KV信息就只需要8（weibo_id）+4（转发数）=12个字节，相比之前有很大的改进。

同时使用一个大的数组来存储计数信息，存储的位置是基于weibo_id的哈希值来计算出来的，具体的算法像下面展示的这样：

![image](/面试题/项目设计场景题/0817-massive-data-counter-design/img-33a1b551f37e.gif)
...
![image](/面试题/项目设计场景题/0817-massive-data-counter-design/img-9400f2938614.gif)
...
![image](/面试题/项目设计场景题/0817-massive-data-counter-design/img-6df7515da579.png)
