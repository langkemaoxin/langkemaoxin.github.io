...
> **Docker 系列 · 第 24/24 篇**
> 上一篇：[《构建进阶——同一个镜像从 1.44GB 滚到 20MB》](/云原生/docker/docker-23-build-advanced)

---

## 开头：改一行配置，业务要断多久？

机器跑得越久，越躲不开三件事：升级 Docker 版本、调日志上限、把数据搬去大盘。这些事最后都落到同一个动作上——**重启 dockerd**。问题是，[第 21 篇](/云原生/docker/docker-21-daemon-runtime/)讲过：真正扛着容器的是 containerd-shim，dockerd 只是「大脑」。那大脑重启，手脚（容器里的业务）必须跟着停吗？
...
这三行不是新面孔：`registry-mirrors` 是[第 4 篇](/云原生/docker/docker-04-install)配的镜像拉取加速；`log-driver` / `log-opts` 是[第 15 篇](/云原生/docker/docker-15-logging-monitoring/)配的日志轮转（单文件 10m、最多留 3 份）。它们背后是一个固定回路，也是后面每一球的固定动作：
...
| `log-driver` / `log-opts` | 全局日志驱动与轮转（[第 15 篇](/云原生/docker/docker-15-logging-monitoring/)） | `docker info` Logging Driver | 雪球 1 对账 |
...
| `insecure-registries` | HTTP 私有仓库白名单；HTTPS + 已信任 CA 的 Harbor 主路径通常不必靠它（细节见[第 10 篇](/云原生/docker/docker-10-harbor)） | `docker info` Insecure Registries | 不动它 |
...
它们还能爬回来，是因为都配了 `restart: unless-stopped`（[第 12 篇](/云原生/docker/docker-12-data-persistence/)）。但 restart 策略只负责「拉回来」，不负责「不中断」：重启那一瞬间连接全断、内存态全丢、数据库要做崩溃恢复。对生产上的有状态服务，这就是每次升级窗口的真实价格。
...
还有个反直觉的细节：重启进行的那几秒，`docker ps` 是**连不上**的——CLI 连的是 dockerd，它正在换班。但宿主机上 `ps` 能看到 containerd-shim 进程还在、业务端口还在响应。[第 21 篇](/云原生/docker/docker-21-daemon-runtime/)画的调用链在这里兑现：容器挂在 shim 下面，不挂在 dockerd 下面。
...
给它配上 Prometheus 抓取任务 + Grafana 面板（或直接用现成的 docker engine dashboard），[第 15 篇](/云原生/docker/docker-15-logging-monitoring/)说的「机器采集」就在 daemon 这一侧闭环了。
...
动手前先 `docker system df` 评估体积（[第 15 篇](/云原生/docker/docker-15-logging-monitoring/)），顺手做一轮计划性 prune，别带着垃圾搬家。
...
安全红线：**远程暴露 daemon = 远程 root**（[第 22 篇](/云原生/docker/docker-22-container-security/)）。优先走 SSH（`ssh://user@host`，免证书管理、有审计）；要开 TCP 就必须配 TLS 双向认证，绝不裸 2375。
...
| 相关篇 | 在这一路上出现的位置 |
|------|----------------------|
| [第 21 篇](/云原生/docker/docker-21-daemon-runtime/) Daemon 与 runtime | 雪球 2、4 的 shim 调用链，雪球 6 的边界判断 |
| [第 12 篇](/云原生/docker/docker-12-data-persistence/) 数据持久化 | 雪球 2 的 restart 策略兜底 |
| [第 4 篇](/云原生/docker/docker-04-install) 安装 | 雪球 1 对账的 registry-mirrors |
| [第 10 篇](/云原生/docker/docker-10-harbor) Harbor | 雪球 1 表里的 insecure-registries |
| [第 15 篇](/云原生/docker/docker-15-logging-monitoring/) 日志与监控 | 雪球 1 的 log-opts、雪球 7 的采集闭环、雪球 8 的 `system df` |
| [第 22 篇](/云原生/docker/docker-22-container-security/) 容器安全 | 雪球 9 的「远程暴露 = 远程 root」 |
| [第 23 篇](/云原生/docker/docker-23-build-advanced) 构建进阶 | 上一篇 |
