...
title: "说说Redis的持久化机制"
sidebarGroup: "Redis"
shortTitle: "说说Redis的持久化机制"
order: 504
date: 2026-06-17
category: "面试题"
tag:
  - "面试题"
description: "简答Redis有两种持久化机制：RDB和AOF。RDB是一种快照持久化的方式，它会将Redis在某个时间点的数据状态以二进制的方式保存到硬盘上的一个文件中。RDB持久化可以通过配置定时或手动触发，也可以设置自动触发的条件。RDB的优点是生成"
article: false
---

> 来源：[说说Redis的持久化机制](https://www.yuque.com/tulingzhouyu/db22bv/lefz0ffra2wp5veg)
...
![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-8f6239bf6f67.png)

![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-0cb5e8c46ce2.png)

![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-ec505c3f3541.png)

![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-ff5431488873.png)

除了执行命令手动触发之外，Redis内部还存在自动触发RDB 的持久化机制，例如以下场景:

1)使用save相关配置,如“save m n”。表示m秒内数据集存在n次修改时，自动触发bgsave。

![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-baf82087d36b.png)

2）如果从节点执行全量复制操作，主节点自动执行bgsave生成RDB文件并发送给从节点。

3)执行debug reload命令重新加载Redis 时，也会自动触发save操作。

![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-30e6679ad05e.png)

4）默认情况下执行shutdown命令时，如果没有开启AOF持久化功能则自动执行bgsave。

![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-58fc198cc8b8.png)

关闭RDB持久化，在课程讲述的Redis版本（6.2.4）上，是将配置文件中的save配置改为 save “”

![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-884623093144.png)

#### bgsave执的行流程

为了快照而暂停写操作，肯定是不能接受的。所以这个时候，Redis 就会借助操作系统提供的写时复制技术（Copy-On-Write, COW），在执行快照的同时，正常处理写操作。

![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-0e97a37ffca6.png)
...
![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-6dd0acc89f89.png)

可以通过执行config set dir {newDir}和config set dbfilename (newFileName}运行期动态执行,当下次运行时RDB文件会保存到新目录。

![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-4fe8ca5af41b.png)
...
![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-c1363efc4d41.png)
...
![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-d79bc1382a9a.png)
...
![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-516f20c9b398.png)

AOF文件名通过appendfilename配置设置，默认文件名是appendonly.aof。保存路径同RDB持久化方式一致，通过dir配置指定。

![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-4f7accc99a12.png)

### AOF的工作流程

AOF的工作流程主要是4个部分:命令写入( append)、文件同步( sync)、文件重写(rewrite)、重启加载( load)。

![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-bfe70c4728b4.png)
...
![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-b54f2aa9deb7.png)
...
![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-91ea14ec641a.png)
...
![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-91e950b859ce.png)

自动触发:根据auto-aof-rewrite-min-size和 auto-aof-rewrite-percentage参数确定自动触发时机。

![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-1c7163a3ecee.png)
...
![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-ead94ad097c9.png)
...
![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-56bab81a23e3.png)

### RDB-AOF混合持久化

通过 `aof-use-rdb-preamble` 配置项可以打开混合开关，yes则表示开启，no表示禁用，默认是禁用的，可通过config set修改

![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-516c7e30f9f9.png)
...
![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-96b2878f690b.png)
...
![image](/面试题/Redis/0504-talk-about-the-persistence-mechanism-of-redis/img-658e58187658.png)
