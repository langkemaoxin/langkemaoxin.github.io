...
> **Docker 系列 · 第 15/24 篇**
> 上一篇：[《HTTPS Nginx——从浏览器红页滚到本机全绿》](/云原生/docker/docker-14-https-nginx) · 下一篇：[《Docker 技术底座——沿着「又轻又像一台机器」逐层解开 Namespace、Cgroups 与 UnionFS》](/云原生/docker/docker-16-tech-foundation)
...
| 🧗 | Prometheus 指标端点 | 只指路，实测放在第 24 篇 |
...
- 往容器里的 `/var/log/app.log` 写文件，`docker logs` 一行没有——那只是[第 12 篇](/云原生/docker/docker-12-data-persistence/)讲过的可写层文件，想读到它只能靠挂载把目录暴露出来，轮转也得自己做；
- Nginx 这类天生写文件日志的软件，官方镜像都做了适配：`nginx` 镜像把 `/var/log/nginx/access.log` 软链到了 `/dev/stdout`——硬把「写文件」掰回「写流」，就是为了进左边那条路。

这套模型来自 [12-Factor](https://12factor.net/zh_cn/logs)：**应用只管往 stdout/stderr 打日志，收集、轮转、转发交给外面**。接日志的是容器里 **PID 1** 进程的两个流——PID 1 与容器生死绑定，[第 19 篇](/云原生/docker/docker-19-process-view/)会对照实验。
...
**容器级**用 `--log-opt` 覆盖全局（也可以写在 Compose 的 `logging:` 字段里，见[第 13 篇](/云原生/docker/docker-13-compose/)）：
...
| 列 | 含义 | 背后机制 |
|------|------|------|
| CPU % | 占宿主机 CPU 的百分比 | cgroups CPU 统计（后文[第 20 篇](/云原生/docker/docker-20-cgroups/)） |
...
- **Containers 的 SIZE 是可写层大小**（[第 12 篇](/云原生/docker/docker-12-data-persistence/)的知识）；
- **Local Volumes 的 RECLAIMABLE 是 0**——命名卷不算可回收（第 12 篇讲过为什么）。
...
`stats`/`events` 是人看的；机器采集走 daemon 的 **Prometheus 指标端点**：daemon.json 配 `"metrics-addr": ":9323"` 后，`/metrics` 暴露容器 CPU/内存/网络等全套指标，配 Prometheus + Grafana（或 cAdvisor）就是完整的容器监控体系。配置和实测放在[第 24 篇 daemon 运维](/云原生/docker/docker-24-daemon-ops/)（涉及 daemon 重启，正好一起讲）。主线到此可以跳过。
...
| 相关篇 | 在这一路上出现的位置 |
|------|----------------------|
| [第 12 篇](/云原生/docker/docker-12-data-persistence/) 持久化 | 雪球 3 写文件日志的挂载自救；雪球 10 可写层与卷的 RECLAIMABLE |
| [第 13 篇](/云原生/docker/docker-13-compose/) Compose | 雪球 5 的容器级轮转还能写进 `logging:` 字段 |
| [第 19 篇](/云原生/docker/docker-19-process-view/) 进程视角 | 雪球 3 的 PID 1 对照实验 |
| [第 20 篇](/云原生/docker/docker-20-cgroups/) CGroup | 雪球 8 stats 每一列的底层机制 |
| [第 24 篇](/云原生/docker/docker-24-daemon-ops/) daemon 运维 | 🧗 Prometheus 端点、daemon.json 的管理 |
...
下一篇：[《Docker 技术底座——沿着「又轻又像一台机器」逐层解开 Namespace、Cgroups 与 UnionFS》](/云原生/docker/docker-16-tech-foundation)。
