---
title: "7.Spring循环依赖源码解析"
sidebarGroup: "Spring 6 源码"
shortTitle: "7.Spring循环依赖源码解析"
order: 7
date: 2025-09-28
category: "源码剖析"
tag:
  - "Spring 6"
  - "源码"
description: "7.Spring循环依赖源码解析"
---

> 来源：[7.Spring循环依赖源码解析](https://www.yuque.com/geren-t8lyq/ru879g/ra3veysbkk4d9ky3)

在线笔记：[https://www.yuque.com/geren-t8lyq/ru879g/ra3veysbkk4d9ky3?singleDoc#](https://www.yuque.com/geren-t8lyq/ru879g/ra3veysbkk4d9ky3?singleDoc#) 《Spring循环依赖源码解析》

流程图：[https://www.processon.com/view/link/5f1fb2cf1e08533a628a7b4c?cid=5f1fb2cf5653bb7fd2566727](https://www.processon.com/view/link/5f1fb2cf1e08533a628a7b4c?cid=5f1fb2cf5653bb7fd2566727)

## 手写循环依赖相关概念：

> **循环依赖问题**： 多个Bean之间形成依赖的闭环，从而会死循环的依赖
> **原因**：依赖注入的过程中，bean还没有创建完（没有一个死循环的出口），导致无法返回依赖的bean。
> 
> 
> **Spring是怎么解决循环依赖的？**
> **第**三级缓存
...
![image.png](/源码剖析/spring6/s6-07-circular-dep/img-001.png)
...
![image.png](/源码剖析/spring6/s6-07-circular-dep/img-002.png)

那么如何打破这个循环，加个中间人（缓存）

![image.png](/源码剖析/spring6/s6-07-circular-dep/img-003.png)
...
![image.png](/源码剖析/spring6/s6-07-circular-dep/img-004.png)
...
1. [https://github.com/spring-projects/spring-framework/pull/26376](https://github.com/spring-projects/spring-framework/pull/26376)
2. [https://github.com/spring-projects/spring-framework/issues/25667](https://github.com/spring-projects/spring-framework/issues/25667)
3. [https://github.com/spring-projects/spring-framework/issues/13117](https://github.com/spring-projects/spring-framework/issues/13117)

### 循环依赖死锁问题：

![image.png](/源码剖析/spring6/s6-07-circular-dep/img-005.png)

把二级缓存放出来可以避免死锁，但是确实会出现多线程getBean获取不完整bean问题， 所以这里只能取舍。

## 循环依赖中@Async问题

**问题根本原因：**@Async没有提前创建动态代理的实现,导致一级缓存和依赖注入属性不一致。

> 6.2+
> **改进不会报错**： 由于Spring6.2支持异步创建， 当出现BeanCurrentlyInCreationException Spring设计者认为这是正常的，觉得是其他的线程正在创建， 所以会忽略。
> 6.2
> **-报错**
![image.png](/源码剖析/spring6/s6-07-circular-dep/img-006.png)

> 解决
> @Lazy在依赖注入的属性上加上@Async： 相当于异常注入， 真正用到的时候再去从容器中获取@Async的bean ,  已经在容器（一级缓存）创建动态代理
