...
- 查看root用户初始密码
  ...
  |                   初始密码                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211124182921234） |

- 登录root用户

  |                登录成功后跳转页面                 |
  | :--------------------------------------: |
  | > （配图缺失：image-20211124183003858） |

- 第一次登录后需要修改密码

  |                   修改密码                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211124193444561） |
...
|                                          |                                          |
| :--------------------------------------: | :--------------------------------------: |
| > （配图缺失：image-20211125141950900） | > （配图缺失：image-20211124200317795） |
...
- 再次重启Jenkins容器，访问Jenkins（需要稍微等会）
  ...
  |                Jenkins首页                 |
  | :--------------------------------------: |
  | > （配图缺失：image-20211124204517433） |
  | > （配图缺失：image-20211124203336300） |
...
- 查看密码登录Jenkins，并登录下载插件
  ...
  |                 登录并下载插件                  |
  | :--------------------------------------: |
  | > （配图缺失：image-20211124205050484） |
  | > （配图缺失：image-20211124205513465） |

- 选择需要安装的插件

  |                选择需要安装的插件                 |
  | :--------------------------------------: |
  | > （配图缺失：image-20211124205854418） |
  | > （配图缺失：image-20211124205858730） |
  | > （配图缺失：image-20211124205917317） |

- 下载完毕设置信息进入首页（可能会出现下载失败的插件）

  |                                          |
  | ---------------------------------------- |
  | > （配图缺失：image-20211124211635550） |
  | > （配图缺失：image-20211124211700999） |
  | > （配图缺失：image-20211124211720836） |
...
- 构建Maven工程发布到GitLab（Gitee、Github均可）
  ...
  |                GitLab查看项目                |
  | :--------------------------------------: |
  | > （配图缺失：image-20211125195818670） |

- Jenkins点击左侧导航新建任务

  |                   新建任务                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211125163541645） |

- 选择自由风格构建任务

  |                   构建任务                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211125170350811） |
...
- 配置任务源码拉取的地址
  ...
  |                   源码管理                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211125170418337） |

- Jenkins立即构建

  |              点击任务test中的立即构建              |
  | :--------------------------------------: |
  | > （配图缺失：image-20211125200218093） |

- 查看构建工程的日志，点击上述③的任务条即可

  |              查看任务拉取Git源码日志               |
  | :--------------------------------------: |
  | > （配图缺失：image-20211125201701443） |
...
- Jenkins配置JDK&Maven并保存
  ...
  |                                          |
  | ---------------------------------------- |
  | > （配图缺失：image-20211125204811759） |
  | > （配图缺失：image-20211125204818869） |

- 配置Jenkins任务构建代码

  |               配置Maven构建代码                |
  | :--------------------------------------: |
  | > （配图缺失：image-20211125205027013） |
  | > （配图缺失：image-20211125205020738） |

- 立即构建测试，查看target下的jar包

  |                   构建源码                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211125205240208） |
  | > （配图缺失：image-20211125205725948） |
...
- 配置任务的构建后操作，发布jar包到目标服务
  ...
  | :--------------------------------------: |
  | > （配图缺失：image-20211125205027013） |
  | > （配图缺失：image-20211125210424346） |
  | > （配图缺失：image-20211125210626631） |

- 立即构建任务，并去目标服务查看

  |                   立即构建                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211125210755556） |
  | > （配图缺失：image-20211125210826057） |
...
- 添加Dockerfile文件
  ...
  |                 构建自定义镜像                  |
  | :--------------------------------------: |
  | > （配图缺失：image-20211126161304485） |

- 添加docker-compose.yml文件

  |               加载自定义镜像启动容器                |
  | :--------------------------------------: |
  | > （配图缺失：image-20211126161331991） |

- 追加Jenkins构建后操作脚本命令

  |               构建后发布并执行脚本命令               |
  | :--------------------------------------: |
  | > （配图缺失：image-20211126161408514） |

- 发布到GitLab后由Jenkins立即构建并托送到目标服务器

  |                   构建日志                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211126161448527） |

- 测试部署到目标服务器程序

  |               查看目标服务器并测试接口               |
  | :--------------------------------------: |
  | > （配图缺失：image-20211126161504715） |
  | > （配图缺失：image-20211126161509411） |
...
- 下载Git Parameter插件
  ...
  |             下载Git Parameter              |
  | :--------------------------------------: |
  | > （配图缺失：image-20211126165209057） |

- 设置项目参数化构建

  |                基于Git标签构建                 |
  | :--------------------------------------: |
  | > （配图缺失：image-20211126165444124） |
  | > （配图缺失：image-20211126172828266） |

- 给项目添加tag版本

  |                 添加tag版本                  |
  | :--------------------------------------: |
  | > （配图缺失：image-20211126165639286） |

- 任务构建时，采用Shell方式构建，拉取指定tag版本代码

  |               切换指定标签并构建项目                |
  | :--------------------------------------: |
  | > （配图缺失：image-20211126174715028） |

- 基于Parameter构建任务，任务发布到目标服务器

  |                   构建任务                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211126174159487） |
...
- 需要设置sysctl.conf文件信息
  ...
  |            设置vm.max_map_count            |
  | :--------------------------------------: |
  | > （配图缺失：image-20211207145215817） |
  | > （配图缺失：image-20211207145342350） |
...
- 重新启动需要一定时间启动，可以可以查看容器日志，看到如下内容代表启动成功
  ...
  |                   容器日志                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211129191426344） |

- 访问Sonar Qube首页

  |                    登录                    |
  | :--------------------------------------: |
  | > （配图缺失：image-20211129191537050） |

- 还需要重新设置一次密码

  |                  重新设置密码                  |
  | :--------------------------------------: |
  | > （配图缺失：image-20211129193824428） |

- Sonar Qube首页

  |               Sonar Qube首页               |
  | :--------------------------------------: |
  | > （配图缺失：image-20211129194148239） |

##### 7.2.2 安装中文插件

|                  安装中文插件                  |
| :--------------------------------------: |
| > （配图缺失：image-20211129194621820） |
...
|                   重启按钮                   |
| :--------------------------------------: |
| > （配图缺失：image-20211129194748765） |

重启后查看效果

|                   首页效果                   |
| :--------------------------------------: |
| > （配图缺失：image-20211129194931944） |
...
- 在代码位置执行命令：mvn sonar:sonar
  ...
  |                  执行代码检测                  |
  | :--------------------------------------: |
  | > （配图缺失：image-20211129195430146） |

- 查看Sonar Qube界面检测结果

  |              Sonar Qube检测结果              |
  | :--------------------------------------: |
  | > （配图缺失：image-20211129195503762） |
...
- 执行命令检测代码
  ...
  |                  查看日志信息                  |
  | :--------------------------------------: |
  | > （配图缺失：image-20211130141303457） |

- 查看SonarQube界面检测结果

  |                   检测结果                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211130144608025） |
...
|              下载Sonar Qube插件              |
| :--------------------------------------: |
| > （配图缺失：image-20211129201625561） |
| > （配图缺失：image-20211129201607240） |
| > （配图缺失：image-20211129202147390） |
...
- 开启Sonar Qube权限验证
  ...
  |             开启Sonar Qube权限校验             |
  | :--------------------------------------: |
  | > （配图缺失：image-20211130144850186） |

- 获取Sonar Qube的令牌

  |                   获取令牌                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211129203102334） |

- 配置Jenkins的Sonar Qube信息

  |                                          |
  | ---------------------------------------- |
  | > （配图缺失：image-20211129203235019） |
  | > （配图缺失：image-20211129203342171） |
  | > （配图缺失：image-20211129203457604） |
...
- 将Sonar-scaner添加到Jenkins数据卷中并配置全局配置
  ...
  |             配置Sonar-scanner              |
  | :--------------------------------------: |
  | > （配图缺失：image-20211130153628925） |

- 配置任务的Sonar-scanner

  |            配置任务的Sonar-scanner            |
  | :--------------------------------------: |
  | > （配图缺失：image-20211130155849143） |

##### 7.4.4 构建任务

|                   构建任务                   |
| :--------------------------------------: |
| > （配图缺失：image-20211130160017465） |
| > （配图缺失：image-20211130160047648） |
...
- 启动Harbor
  ...
  |                   查看日志                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211130215941857） |

- 登录Harbor

  |                 登录Harbor                 |
  | :--------------------------------------: |
  | > （配图缺失：image-20211130220028840） |

- 首页信息

  |                   首页信息                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211130220111602） |
...
- 创建用户
  ...
  |                   创建用户                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211201213427157） |

- 构建项目（设置为私有）

  |                   构建项目                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211201213751780） |

- 给项目追加用户

  |                  追加用户管理                  |
  | :--------------------------------------: |
  | > （配图缺失：image-20211201213832458） |

- 切换测试用户

  |                  切换测试用户                  |
  | :--------------------------------------: |
  | > （配图缺失：image-20211201214008303） |
...
- 推送镜像到Harbor
  ...
  |               推送镜像到Harbor                |
  | :--------------------------------------: |
  | > （配图缺失：image-20211201221225196） |
  | > （配图缺失：image-20211201221300055） |
...
- 编写脚本文件，添加到目标服务器
  ...
  |                    如图                    |
  | :--------------------------------------: |
  | > （配图缺失：image-20211203192047357） |

##### 8.3.7 配置构建后操作

|                  执行脚本文件                  |
| :--------------------------------------: |
| > （配图缺失：image-20211229155949038） |
...
- 构建任务
  ...
  | 构建Jenkins流水线任务                           |
  | ---------------------------------------- |
  | > （配图缺失：image-20211202144429302） |

- 生成Groovy脚本

  |             Hello World脚本生成              |
  | :--------------------------------------: |
  | > （配图缺失：image-20211202144531749） |

- 构建后查看视图

  |                 构建后查看视图                  |
  | :--------------------------------------: |
  | > （配图缺失：image-20211202144616117） |
...
- 编写例子测试
  ...
  |                配置Grovvy脚本                |
  | :--------------------------------------: |
  | > （配图缺失：image-20211202145155428） |

- 查看效果

  |                   查看效果                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211202145240166） |

[Ps：涉及到特定脚本，Jenkins给予了充足的提示，可以自动生成命令]()

|                  生成命令位置                  |
| :--------------------------------------: |
| > （配图缺失：image-20211202145349043） |
...
- 配置pipeline
  ...
  |                配置pipeline                |
  | :--------------------------------------: |
  | > （配图缺失：image-20211202151127254） |

- 准备Jenkinsfile

  |             准备Jenkinsfile文件              |
  | :--------------------------------------: |
  | > （配图缺失：image-20211202151155145） |

- 测试效果

  |                   测试效果                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211202151225161） |
...
|                   语法生成                   |
| :--------------------------------------: |
| > （配图缺失：image-20211202192047619） |

> （配图缺失：image-20211202192129895）将*/master更改为标签[${​tag}]()
...
- 安装插件
  ...
  |                   安装插件                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211209151549412） |

- 钉钉内部创建群组并构建机器人

  |              钉钉内部创建群组并构建机器人              |
  | :--------------------------------------: |
  | > （配图缺失：image-20211209152217433） |
  | > （配图缺失：image-20211209152252050） |
  | > （配图缺失：image-20211209152403312） |
...
- 查看启动情况
  ...
  |                   查看效果                   |
  | :--------------------------------------: |
  | > （配图缺失：image-20211213184701784） |

- 在浏览器中打开链接 http://your-node-ip-address:30080

  |                    首页                    |
  | :--------------------------------------: |
  | > （配图缺失：image-20211213184742709） |
...
- 命令实现效果
  ...
  |           kubectl get service            |
  | :--------------------------------------: |
  | > （配图缺失：image-20220104214659229） |

  也可以通过`Deployment名称.namespace名称.svc`作为域名访问

  |                 在服务容器内执行                 |
  | :--------------------------------------: |
  | > （配图缺失：image-20220104215030265） |
...
- 命令实现方式
  ...
  |               查看Service效果                |
  | :--------------------------------------: |
  | > （配图缺失：image-20220104222750733） |
  | > （配图缺失：image-20220104222819455） |
...
|                Kuboard安装                 |
| :--------------------------------------: |
| > （配图缺失：image-20220105153343642） |
| **> （配图缺失：image-20220105153416367）** |

因为副本数默认为1，但是k8s整体集群就2个节点，所以显示下面即为安装成功

|                   安装成功                   |
| :--------------------------------------: |
| > （配图缺失：image-20220105153502619） |
...
|                 配置hosts                  |
| :--------------------------------------: |
| > （配图缺失：image-20220105204921272） |

记下来既可以访问在Service中暴露的Nginx信息

|               服通过Ingress访问               |
| :--------------------------------------: |
| > （配图缺失：image-20220105205407393） |
